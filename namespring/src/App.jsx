...
function App() {
  ...
  const handleAnalyze = () => {
    const isComplete = !selectedSurnameEntries.includes(null) && !selectedFirstNameEntries.includes(null);
    if (!isComplete) { alert("모든 한자를 선택해주세요."); return; }

    const [year, month, day] = birthDate.split('-').map(Number);
    const [hour, minute] = birthTime.split(':').map(Number);

    // SeedTs 엔진이 요구하는 UserInfo 인터페이스 구성
    const userInfo = {
      ...
      <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100 w-full max-w-2xl overflow-hidden">
        ...
        {!analysisResult && (
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <button onClick={handleAnalyze} className="w-full py-6 bg-slate-900 text-white rounded-[2rem] font-black text-lg hover:bg-black transition-all">ANALYZE NAME</button>
          </div>
        )}

        {/* 결과 보고서 섹션 */}
        {analysisResult && (
          <NamingReport result={analysisResult.candidates[0]} />
        )}
      </div>
