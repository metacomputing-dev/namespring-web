    ...
    <div className="min-h-screen p-4 md:p-8 font-sans text-[var(--ns-text)]">
      <div className="max-w-7xl mx-auto">
        <div className="bg-[var(--ns-surface)] border border-[var(--ns-border)] rounded-3xl p-6 mb-6">
          ...
          <h1 className="text-2xl font-black text-[var(--ns-accent-text)] mb-4">NameStat Shard DB Viewer</h1>
          <div className="flex flex-wrap items-center gap-3">
            ...
            {dbInfo.fileName && (
              <span className="text-xs font-semibold text-[var(--ns-muted)]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-4 space-y-4">
            <div className="bg-[var(--ns-surface)] border border-[var(--ns-border)] rounded-2xl p-4">
              <input
                type="text"
                value={searchTerm}
              ...
              {filtered.map((item) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 border-b border-[var(--ns-border)] hover:bg-[var(--ns-surface-soft)] ${
                    selected?.id === item.id ? 'bg-[var(--ns-surface-soft)]' : ''
                  }`}
                >
                  <p className="font-bold text-[var(--ns-accent-text)]">{item.name}</p>
                  <p className="text-xs text-[var(--ns-muted)]">초성 {item.first_choseong} / 첫글자 {item.first_char}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-8">
            ...
            {selected && (
              <div className="bg-[var(--ns-surface)] border border-[var(--ns-border)] rounded-2xl p-6 space-y-5">
                <div>
                  <p className="text-xs text-[var(--ns-muted)] font-black uppercase tracking-widest mb-1">Name</p>
                  <h2 className="text-3xl font-black text-[var(--ns-accent-text)]">{selected.name}</h2>
                  <p className="text-sm text-[var(--ns-muted)] mt-1">first_char: {selected.first_char} / first_choseong: {selected.first_choseong}</p>
                </div>

                <section>
                  <h3 className="font-black text-[var(--ns-accent-text)] mb-2">similar_names</h3>
                  <div className="flex flex-wrap gap-2">
                    {(selected.similar_names || []).slice(0, 80).map((n) => (
                      <span key={n} className="px-3 py-1.5 bg-[var(--ns-surface-soft)] text-[var(--ns-text)] text-xs font-semibold rounded-lg">{n}</span>
                    ))}
                  </div>
                </section>

                <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-[var(--ns-surface-soft)] rounded-xl p-4">
                    <h3 className="font-black text-[var(--ns-accent-text)] mb-2">yearly_rank</h3>
                    <pre className="text-xs text-[var(--ns-muted)] overflow-auto max-h-56">{JSON.stringify(selected.yearly_rank, null, 2)}</pre>
                  </div>
                  <div className="bg-[var(--ns-surface-soft)] rounded-xl p-4">
                    <h3 className="font-black text-[var(--ns-accent-text)] mb-2">yearly_birth</h3>
                    <pre className="text-xs text-[var(--ns-muted)] overflow-auto max-h-56">{JSON.stringify(selected.yearly_birth, null, 2)}</pre>
                  </div>
                </section>

                <section>
                  <h3 className="font-black text-[var(--ns-accent-text)] mb-2">hanja_combinations</h3>
                  <pre className="text-xs text-[var(--ns-muted)] bg-[var(--ns-surface-soft)] rounded-xl p-4 overflow-auto max-h-56">
