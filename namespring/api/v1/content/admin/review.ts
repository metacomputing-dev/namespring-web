import type { ReviewContentArtifactRequestV1 } from "../../../../shared/types/content-lifecycle.js";
...
import { parseReviewContentArtifactRequest } from "../../../_lib/content-validation.js";
...
export default async function handler(req: AuthRequestLike, res?: AuthNodeResponseLike) {
  try {
    ...
    const request = parseReviewContentArtifactRequest(await readJsonBody<ReviewContentArtifactRequestV1>(req));
    return sendAuthJson(res, 200, await getContentLifecycleService().review(actor, request));
  } catch (error) {
