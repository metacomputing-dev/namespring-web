import type { RollbackContentArtifactRequestV1 } from "../../../../shared/types/content-lifecycle.js";
...
import { parseRollbackContentArtifactRequest } from "../../../_lib/content-validation.js";
...
export default async function handler(req: AuthRequestLike, res?: AuthNodeResponseLike) {
  try {
    ...
    const request = parseRollbackContentArtifactRequest(await readJsonBody<RollbackContentArtifactRequestV1>(req));
    return sendAuthJson(res, 200, await getContentLifecycleService().rollback(actor, request));
  } catch (error) {
