import type { GrantSyncConsentRequestV1 } from "../../../shared/types/sync-contract.js";
...
import { parseGrantSyncConsentRequest } from "../../_lib/sync-validation.js";
...
export default async function handler(req: AuthRequestLike, res?: AuthNodeResponseLike) {
  try {
    ...
    const request = parseGrantSyncConsentRequest(await readJsonBody<GrantSyncConsentRequestV1>(req));
    return sendAuthJson(res, 200, await getAccountSyncService().grantConsent(actor, request));
  } catch (error) {
