import type { DeleteSyncedDataRequestV1 } from "../../../shared/types/sync-contract.js";
...
import { parseDeleteSyncedDataRequest } from "../../_lib/sync-validation.js";
...
export default async function handler(req: AuthRequestLike, res?: AuthNodeResponseLike) {
  try {
    ...
    const request = parseDeleteSyncedDataRequest(await readJsonBody<DeleteSyncedDataRequestV1>(req));
    return sendAuthJson(res, 200, await getAccountSyncService().deleteData(actor, request));
  } catch (error) {
