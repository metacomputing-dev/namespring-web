...
function sinDeg(xDeg: number): number {
...
const L4: readonly VsopTerm[] = [
  [114.0, 3.142, 0.0],
  [8.0, 4.13, 6283.08],
  [1.0, 3.84, 12566.15],
];

const L5: readonly VsopTerm[] = [[1.0, 3.14, 0.0]];

// VSOP87 Earth heliocentric distance periodic terms (R0..R4).
//
// Used by the opt-in R-corrected aberration model. Coefficients are the
// 59 terms listed in NREL/TP-560-34302 (Reda & Andreas), Tables A.4.5–
// A.4.9, identical to pvlib.spa's `R_TERMS` (BSD-3-Clause).
//
// Earth-Sun distance R (in AU) is recovered as
//   R(τ) = (R0(τ) + R1(τ)·τ + R2(τ)·τ² + R3(τ)·τ³ + R4(τ)·τ⁴) / 1e8
// where τ = (JDE − 2451545.0) / 365250 is the same Julian millennium
// argument used by the L-series above.
//
// Not yet wired into the longitude pipeline; the next commits introduce
// the rest of the series, the helper, the config flag, and the
// dispatch into `solarApparentLongitudeDeg`.
const R0: readonly VsopTerm[] = [
  [100013989.0, 0.0, 0.0],
...
const R3: readonly VsopTerm[] = [
  [145.0, 4.273, 6283.076],
  [7.0, 3.92, 12566.15],
];

const R4: readonly VsopTerm[] = [[4.0, 2.56, 6283.08]];

function sumVsopTerms(terms: readonly VsopTerm[], x: number): number {
  let s = 0;
...
function earthHeliocentricLongitudeDeg(jme: number): number {
 ...
 */
function deltaTSeconds(year: number, month: number): number {
...
function deltaTSecondsFromJulianDayUtc(jdUtc: number): number {
 ...
 * Astronomical Algorithms 2nd ed., eq. 25.2.
 *
 * (Meeus also has eq. 28.2, which uses τ = T / 10 — Julian millennia
 * — and adds T³, T⁴, T⁵ terms for sub-mas accuracy. We deliberately
 * use 25.2 here because the consumer cares about minute-level Equation
 * of Time, where the residual of 25.2 is well below 1 ms.)
 *
 * Used by the precise Equation-of-Time path; not yet wired.
 */
export function sunMeanLongitudeDeg(T: number): number {
  const T2 = T * T;
  return mod360(280.46646 + 36000.76983 * T + 0.0003032 * T2);
}
