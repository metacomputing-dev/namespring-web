 ...
 * 십신을 다섯 쌍으로 묶은 그룹 이름.
 */
export type SipsinGroup = 'BIGEOB' | 'SIKSANG' | 'JAE' | 'GWAN' | 'IN';

/**
 * 궁위(宮位) — 사주 네 기둥의 위치
 *
 * 년주(NYEON), 월주(WOL), 일지(ILJI), 시주(SI)
 */
export type Gungwi = 'NYEON' | 'WOL' | 'ILJI' | 'SI';
 ...
 *
 * @example
 * sipsinGroupOf('SIK_SHIN') // 'SIKSANG'
 * sipsinGroupOf('JEONG_JAE') // 'JAE'
 */
export function sipsinGroupOf(sipsin: Sipsin): SipsinGroup {
  for (const [key, members] of Object.entries(SIPSIN_GROUPS)) {
    if ((members as Sipsin[]).includes(sipsin)) return key as SipsinGroup;
  }
  // 이론상 도달 불가하나 타입 안전을 위해
  return 'BIGEOB';
}
 ...
 * 중성(中性) 십신
 *
 * 비견(比肩)과 편재(偏財)는 사주 구조에 따라
 * 길신 또는 흉신의 역할이 유동적이다.
 * 비견은 독립·자주의 별이지만 재성을 분산하며,
 * 편재는 사업 수완의 별이지만 변동과 투기의 위험이 있다.
 */
export const JUNGSEONG_SIPSIN: Sipsin[] = [
  'BI_GYEON',    // 비견 — 독립과 재분산 사이
  'PYEON_JAE',   // 편재 — 사업 수완과 투기 사이
];
...
export const SIPSIN_COMBINATIONS: Array<{
  ...
  {
    combination: ['PYEON_JAE', 'PYEON_GWAN'],
    meaning:
      '재살(財殺) — ' +
      '편재가 편관(칠살)을 생하는 구조. 재물이 권력의 원천이 되어 강력한 ' +
      '사업형 리더십을 발휘할 수 있으나, 칠살이 과강하면 흉작용이 증폭된다.',
    condition:
      '일간이 충분히 강하여 칠살의 극을 감당할 수 있어야 하며, ' +
      '식신이 칠살을 제어하면 최상의 사업 귀격이 된다.',
  },
  {
    combination: ['JEONG_IN', 'JEONG_JAE'],
    meaning:
      '재인교전(財印交戰) — ' +
      '정재가 정인을 극하여 학문·보호의 기운이 약화되는 구조. ' +
      '학업 중단, 어머니와의 불화, 자격·면허 취득의 어려움이 따른다.',
    condition:
      '재성과 인성이 동주(同柱) 또는 인접 주에 함께 있을 때 교전이 성립한다. ' +
      '관성이 있으면 재생관(財生官), 관생인(官生印)으로 통관(通關)하여 해소된다.',
  },
];
 ...
 *
 * 십신과 인척 관계의 완전한 매핑.
 * 남명(男命)과 여명(女命) 각각의 관점에서 정리한다.
 *
 * 전통 육친론의 핵심 원칙:
 *   - 비겁(比劫) = 형제·자매 (나와 같은 오행)
 *   - 식상(食傷) = 남: 장모·조모, 여: 자녀 (내가 생하는 오행)
 *   - 재성(財星) = 남: 처·아버지, 여: 아버지·시부 (내가 극하는 오행)
 *   - 관성(官星) = 남: 자녀(딸·아들), 여: 남편 (나를 극하는 오행)
 *   - 인성(印星) = 어머니 (나를 생하는 오행)
 *
 * 참고: 육친 배속은 남녀에 따라 다르며, 편(偏)·정(正)에 따라서도
 * 구체적인 인척이 달라진다.
 */
export const YUK_CHIN_TABLE: Record<Sipsin, {
  /** 남명(男命) 기준 육친 */
  male: string;
  /** 여명(女命) 기준 육친 */
  female: string;
  /** 육친 논리 — 왜 이 십신이 해당 육친인지의 논리 */
  logic: string;
}> = {
  BI_GYEON: {
    male:   '형제(兄弟)·친구·동료',
    female: '자매(姉妹)·친구·동료',
    logic:  '나와 같은 오행, 같은 음양 = 어깨를 나란히 하는 형제(비견)의 뜻',
  },
  GEOB_JAE: {
    male:   '이복형제(異腹兄弟)·의형제',
    female: '이복자매·경쟁 상대',
    logic:  '나와 같은 오행, 다른 음양 = 재물을 나눠 갖는(겁탈하는) 이복형제의 뜻',
  },
  SIK_SHIN: {
    male:   '장모(丈母)·할머니(외조모)',
    female: '자녀(子女) — 딸',
    logic:
      '남: 처(정재)의 어머니(인성) = 정재에서 인성으로 두 단계. ' +
      '여: 내가 낳은(생한) 존재 = 자녀. 같은 음양이므로 딸.',
  },
  SANG_GWAN: {
    male:   '할머니(외조모)·손아래 형제',
    female: '자녀(子女) — 아들',
    logic:
      '남: 아버지(편재)의 어머니(인성). ' +
      '여: 내가 낳은 존재이나 다른 음양이므로 아들.',
  },
  PYEON_JAE: {
    male:   '아버지(父)·첩(妾)·애인',
    female: '시아버지(媤父)·아버지(父)',
    logic:
      '남: 정인(어머니)을 극하는 존재 = 편재가 아버지. 동극 재성이므로 편처·첩. ' +
      '여: 정관(남편)의 아버지 = 시아버지.',
  },
  JEONG_JAE: {
    male:   '처(妻)·정식 배우자',
    female: '시아버지(媤父)·아버지(父)',
    logic:
      '남: 내가 극하되 음양이 다른 정재 = 정식 배우자(처). ' +
      '여: 편재와 같은 논리로 시부·부.',
  },
  PYEON_GWAN: {
    male:   '아들(子)',
    female: '남편(夫) — 정식 외 관계, 애인·재혼 남편',
    logic:
      '남: 나를 극하는 동극 관성 = 자식 중 아들(편관). ' +
      '여: 나를 제약하는 존재이되 동극이므로 비공식적 남성 인연.',
  },
  JEONG_GWAN: {
    male:   '딸(女兒)',
    female: '남편(夫) — 정식 배우자',
    logic:
      '남: 나를 극하는 이극 관성 = 자식 중 딸(정관). ' +
      '여: 나를 제약하되 음양이 달라 조화로운 관계 = 정식 남편.',
  },
  PYEON_IN: {
    male:   '계모(繼母)·이복어머니·이모',
    female: '계모·이복어머니·시어머니',
    logic:
      '나를 생하되 동극인 인성 = 친어머니가 아닌 이복·계모. ' +
      '효신(梟神)이라 불리며 식신을 극하는 관계.',
  },
  JEONG_IN: {
    male:   '어머니(母)·외삼촌',
    female: '어머니(母)·외삼촌',
    logic:
      '나를 생하되 이극인 인성 = 나를 낳아준 친어머니. ' +
      '인수(印綬)라 하여 보호·양육의 상징.',
  },
};
 ...
 * 궁위(宮位)별 십신 해석 테이블
 *
 * 사주의 네 기둥(년주·월주·일지·시주)에 각 십신이 놓일 때의 해석.
 *
 * 궁위(宮位)란 사주 네 기둥이 상징하는 시공간적 의미이다:
 *   - 년주(年柱): 조상궁(祖上宮) — 조부모·가문·유년기(0~15세)·사회적 환경
 *   - 월주(月柱): 부모궁(父母宮) — 부모·형제·청년기(16~30세)·직업 환경
 *   - 일지(日支): 배우자궁(配偶宮) — 배우자·가정·중년기(31~45세)·내면
 *   - 시주(時柱): 자녀궁(子女宮) — 자녀·말년·노년기(46세~)·결과
 *
 * 주의: 십신의 궁위별 해석은 전체 사주 구조의 맥락에서 이해해야 한다.
 */
export const SIPSIN_GUNGWI: Record<Sipsin, Record<Gungwi, string>> = {
  BI_GYEON: {
    NYEON: '조부모 대에 형제·동기가 많거나 독립심 강한 가문. 유년기에 경쟁 환경에 놓인다.',
    WOL:   '형제·친구·동료와 인연이 깊다. 직업적으로 독립·자영업 성향이 강하며 동업 기회가 많다.',
    ILJI:  '배우자가 독립적이고 자기 주관이 뚜렷하다. 배우자와 대등한 관계를 형성하며 때로 갈등이 생긴다.',
    SI:    '말년에 형제·친구와 함께하거나 독립적인 활동을 이어간다. 자녀가 독립심이 강하다.',
  },
  GEOB_JAE: {
    NYEON: '조부모 대에 재물 분쟁이 있었거나 가산이 흩어진 역사가 있다.',
    WOL:   '형제·동료 간 재물 경쟁이 심하다. 직업상 경쟁이 치열한 환경에 놓인다.',
    ILJI:  '배우자와 재물 문제로 갈등이 생길 수 있다. 배우자가 활동적이나 충동적인 면이 있다.',
    SI:    '말년에 재물 분산이 우려된다. 자녀가 투기적이거나 사업 의지가 강하다.',
  },
  SIK_SHIN: {
    NYEON: '조부모 대에 먹복이 있었으며 풍족한 환경에서 성장한다. 유년기에 재능이 일찍 발현된다.',
    WOL:   '직업에서 창의성·표현력을 활용한다. 예술·요리·서비스업과 인연이 깊다.',
    ILJI:  '배우자가 온화하고 식복(食福)이 있다. 가정적이며 먹는 즐거움을 공유한다.',
    SI:    '말년에 여유롭고 풍족한 삶을 영위한다. 자녀 복이 있으며(특히 여명에서 딸 복).',
  },
  SANG_GWAN: {
    NYEON: '유년기에 반항심이 강하거나 학교·권위와 충돌하는 경험이 있다.',
    WOL:   '직업적으로 뛰어난 재능을 발휘하나 직장 상사와의 마찰이 잦다. 프리랜서·전문직에 적합.',
    ILJI:  '배우자가 재능이 뛰어나나 자존심이 세다. 배우자와 표현 방식의 차이로 갈등 가능.',
    SI:    '말년에 예술·학문으로 이름을 남긴다. 자녀가 총명하나 반항적일 수 있다(특히 여명에서 아들).',
  },
  PYEON_JAE: {
    NYEON: '부유한 가문에서 태어나거나 아버지의 사업 영향을 받는다.',
    WOL:   '사업·투자·영업 분야에서 두각을 나타낸다. 인맥이 넓고 활동 범위가 크다.',
    ILJI:  '남명: 배우자 외의 여성 인연이 있을 수 있다. 여명: 가정 내 경제권을 쥔다.',
    SI:    '말년에 사업 확장 또는 재물 변동이 있다. 노후 재물 운이 활발하다.',
  },
  JEONG_JAE: {
    NYEON: '근면한 가정에서 태어나 경제적으로 안정된 유년기를 보낸다.',
    WOL:   '안정적인 직업에 종사하며 성실한 재물 축적이 가능하다. 봉급생활에 적합.',
    ILJI:  '남명: 처와의 인연이 좋으며 가정적이다. 여명: 가정 경제가 안정적이다.',
    SI:    '말년에 안정적 노후를 보내며 재물이 축적된다. 자녀가 성실하다.',
  },
  PYEON_GWAN: {
    NYEON: '어린 시절 엄격한 환경에서 성장하거나 권위에 눌리는 경험이 있다.',
    WOL:   '직업적으로 군인·경찰·법조계 등 강직한 직종과 인연이 깊다. 직장에서 압박이 있다.',
    ILJI:  '배우자가 강하고 카리스마가 있다. 여명: 비공식적 남성 인연 또는 재혼의 가능성.',
    SI:    '말년에 자녀(아들)와의 관계가 역동적이다. 남명: 아들이 강한 성품을 가진다.',
  },
  JEONG_GWAN: {
    NYEON: '명문 가정에서 태어나거나 사회적으로 존경받는 가문의 영향을 받는다.',
    WOL:   '공직·관료·대기업 등 사회적으로 안정되고 명예로운 직업에 종사한다.',
    ILJI:  '여명: 남편이 사회적 지위가 높고 품격이 있다. 남명: 배우자 관계가 안정적이다.',
    SI:    '말년에 명예와 사회적 존경을 받는다. 자녀가 사회적으로 성공한다.',
  },
  PYEON_IN: {
    NYEON: '유년기에 계모·이모 등 비전형적 양육 환경을 경험하거나, 독특한 가풍이 있다.',
    WOL:   '특수 분야·비주류 직종에서 능력을 발휘한다. 종교·철학·역술 관련 직업과 인연.',
    ILJI:  '배우자가 독특한 개성을 가지며, 영적·철학적 교류가 있는 관계.',
    SI:    '말년에 종교·철학·학문에 몰두한다. 자녀와의 관계가 특이하거나 식복이 약해질 수 있다.',
  },
  JEONG_IN: {
    NYEON: '어머니의 보호를 받으며 학문적 환경에서 성장한다. 가문에 학자·교육자가 많다.',
    WOL:   '교직·학계·자격 전문직에 종사한다. 학문을 통한 입신양명(立身揚名)이 기대된다.',
    ILJI:  '배우자가 학식이 높고 인자하다. 가정 내에서 학문·교육 분위기가 강하다.',
    SI:    '말년에 학문적 성취를 이루거나 후학을 양성한다. 자녀가 학업에 뛰어나다.',
  },
};
...
/**
 * 십신 과다(過多)·부족(不足) 해석 테이블
 *
 * 사주 팔자에서 특정 십신이 3개 이상이면 과다(過多),
 * 하나도 없으면 부족(不足)으로 판단한다.
 *
 * 과다·부족의 영향은 일간의 강약(신강·신약)에 따라 달라진다.
 */
export const SIPSIN_GWADA_BUJOK: Record<Sipsin, {
  /** 과다(過多) — 3개 이상일 때의 해석 */
  gwada: string;
  /** 부족(不足) — 하나도 없을 때의 해석 */
  bujok: string;
  ...
  BI_GYEON: {
    gwada:
      '비견 과다: 독립심과 자존심이 지나쳐 타인과의 조화가 어렵다. ' +
      '재물이 분산되어 모이지 않고, 형제·동료 간 경쟁과 갈등이 빈번하다. ' +
      '신강 사주에서는 관성(官星)·재성(財星)이 용신으로 필요하다.',
    bujok:
      '비견 부재: 의지할 형제·동료가 없어 외로우며, 모든 것을 혼자 해결해야 한다. ' +
      '신약 사주에서 비견이 없으면 일간의 힘이 더욱 약화된다.',
  },
  GEOB_JAE: {
    gwada:
      '겁재 과다: 충동적이고 투기적 성향이 강해져 재물 손실이 잦다. ' +
      '남명에서는 처와의 불화, 여명에서는 경쟁적 여성 관계가 복잡해진다. ' +
      '관성(官星)의 제화가 필수적이다.',
    bujok:
      '겁재 부재: 추진력과 결단력이 부족하여 기회를 놓치기 쉽다. ' +
      '신약 사주에서는 행동력 부족으로 원하는 바를 이루기 어렵다.',
  },
  SIK_SHIN: {
    gwada:
      '식신 과다: 먹복은 넘치나 게으름과 나태함에 빠지기 쉽다. ' +
      '지나친 여유로 인해 추진력이 약화되고, 일간의 기운이 설기(洩氣)되어 신약해질 수 있다. ' +
      '관성이 약화되어 직업·명예에 문제가 생길 수 있다.',
    bujok:
      '식신 부재: 먹복(食福)이 약하고 창의적 표현력이 부족하다. ' +
      '편관(칠살)을 제어할 식신이 없어 칠살의 흉작용이 드러날 수 있다.',
  },
  SANG_GWAN: {
    gwada:
      '상관 과다: 총명하나 교만하고 입이 거칠어진다. ' +
      '관성을 극하여 직장 생활이 어렵고 관재구설이 끊이지 않는다. ' +
      '여명에서는 남편과의 심한 불화가 우려된다. 인성(印星)의 제화가 필요하다.',
    bujok:
      '상관 부재: 자기 표현이 서투르고 창의적 돌파구를 찾기 어렵다. ' +
      '예술적 재능의 발현이 약하며, 비판 정신이 부족하여 불합리함에 순응할 수 있다.',
  },
  PYEON_JAE: {
    gwada:
      '편재 과다: 재물이 들어오지만 나가는 것도 많아 변동이 심하다. ' +
      '남명에서는 여성 관계가 복잡해지며, 투기·사행심에 빠지기 쉽다. ' +
      '비겁이 재성을 감당할 만큼 일간이 강해야 한다.',
    bujok:
      '편재 부재: 사업적 수완과 대인관계 능력이 부족하다. ' +
      '아버지와의 인연이 약하거나 아버지 덕이 부족할 수 있다.',
  },
  JEONG_JAE: {
    gwada:
      '정재 과다: 소유욕이 지나쳐 인색해지고 재물에 집착한다. ' +
      '인성(印星)을 극하여 학업·어머니와의 관계에 문제가 생긴다. ' +
      '일간이 약하면 재다신약(財多身弱)으로 재물을 감당하지 못한다.',
    bujok:
      '정재 부재: 안정적 재물 축적이 어렵고, 남명에서는 배우자 인연이 늦거나 약하다. ' +
      '근면한 저축 습관이 형성되기 어렵다.',
  },
  PYEON_GWAN: {
    gwada:
      '편관 과다: 신체적 위험, 사고, 관재구설이 잦다. ' +
      '일간이 약하면 건강 문제와 직업 불안이 심각해진다. ' +
      '식신 또는 인성의 제화가 반드시 필요하다. 여명에서는 남성 인연이 복잡하다.',
    bujok:
      '편관 부재: 외부의 제약·단련이 없어 자기 관리가 느슨해질 수 있다. ' +
      '남명에서는 아들 복이 약할 수 있다.',
  },
  JEONG_GWAN: {
    gwada:
      '정관 과다: 관다신약(官多身弱) — 정관이 많으면 오히려 편관(칠살)처럼 작용한다. ' +
      '사회적 압박이 과중하여 자유롭지 못하고, 체면에 얽매여 스트레스가 심하다.',
    bujok:
      '정관 부재: 사회적 명예와 직업 안정성이 약하다. ' +
      '여명에서는 남편 인연이 늦거나 배우자 덕이 부족할 수 있다.',
  },
  PYEON_IN: {
    gwada:
      '편인 과다: 고독하고 편벽한 사고에 빠지며 타인과의 소통이 어렵다. ' +
      '식신을 극하여 도식(倒食)이 되면 먹복·건강·직업이 모두 약화된다. ' +
      '재성으로 편인을 제화해야 한다.',
    bujok:
      '편인 부재: 독창적 사고와 영적 감수성이 약하다. ' +
      '특수 분야에서의 전문성을 발휘하기 어렵다.',
  },
  JEONG_IN: {
    gwada:
      '정인 과다: 의존적이고 나태해져 자기 계발이 부족해진다. ' +
      '신강 과다(過多)로 일간이 너무 강해져 식상(食傷)의 발현이 약화된다. ' +
      '학문만 추구하고 실행력이 떨어진다.',
    bujok:
      '정인 부재: 어머니 덕이 부족하거나 학문적 지원이 약하다. ' +
      '자격증·면허 취득이 어렵고, 정신적 안정감이 약할 수 있다.',
  },
};
...
/**
 * 십신의 합(合)·충(沖)에 의한 변화 해설
 *
 * 천간합(天干合)·지지충(地支沖) 등으로 인해 십신의 작용이
 * 변화하거나 소멸·강화되는 경우를 서술한다.
 *
 * 핵심 원리:
 *   - 합(合): 두 십신이 합하면 본래의 성질이 변화하거나 중화된다.
 *     합거(合去) — 용신이 합으로 제거되면 흉, 기신이 합으로 제거되면 길.
 *   - 충(沖): 두 십신이 충돌하면 한쪽 또는 양쪽의 힘이 약화된다.
 *     충거(沖去) — 왕한 쪽이 약한 쪽을 충으로 제거한다.
 *   - 형(刑): 자형(自刑)·삼형(三刑) 등으로 십신이 손상된다.
 */
export const SIPSIN_HAP_CHUNG: Array<{
  /** 변화의 유형 */
  type: 'HAP' | 'CHUNG' | 'HYEONG';
  /** 관련 십신 */
  sipsins: Sipsin[];
  /** 변화 해설 */
  description: string;
  ...
  // -- 합(合)에 의한 변화 --
  {
    ...
    sipsins: ['JEONG_GWAN'],
    description:
      '정관합거(正官合去) — 정관이 천간합으로 합거되면 명예와 직업이 약화된다. ' +
      '여명에서 정관이 합거되면 남편 인연에 문제가 생긴다. ' +
      '예: 甲일간의 정관 辛이 丙과 합하여 합거되면 관성의 기능이 소멸한다.',
  },
  {
    type: 'HAP',
    sipsins: ['PYEON_GWAN'],
    description:
      '합살유관(合殺留官) — 칠살(편관)이 합하여 제거되면 정관의 기능만 남아 길해진다. ' +
      '예: 甲일간의 칠살 庚이 乙과 합하여 합거되면 칠살의 위협이 사라진다.',
  },
  {
    type: 'HAP',
    sipsins: ['JEONG_JAE'],
    description:
      '정재합거(正財合去) — 정재가 합거되면 안정적 재물이 흔들린다. ' +
      '남명에서 정재가 합거되면 처(배우자)와의 인연에 이상이 생긴다.',
  },
  {
    type: 'HAP',
    sipsins: ['JEONG_IN'],
    description:
      '정인합거(正印合去) — 정인이 합거되면 학문·보호의 기운이 약화된다. ' +
      '어머니와의 인연이 약해지거나 자격·면허 취득에 어려움이 따른다.',
  },
  {
    type: 'HAP',
    sipsins: ['SIK_SHIN'],
    description:
      '식신합거(食神合去) — 식신이 합거되면 먹복과 재능 발휘가 약화된다. ' +
      '편관을 제어할 식신이 사라져 칠살의 흉작용이 드러날 수 있다.',
  },
  ...
  // -- 충(沖)에 의한 변화 --
  {
    ...
    sipsins: ['JEONG_GWAN'],
    description:
      '정관충거(正官沖去) — 정관의 지지가 충으로 깨지면 직업·명예가 불안정해진다. ' +
      '관직에서의 이동·좌천·실직 등이 우려된다.',
  },
  {
    type: 'CHUNG',
    sipsins: ['JEONG_JAE', 'PYEON_JAE'],
    description:
      '재성충(財星沖) — 재성의 지지가 충돌하면 재물 변동이 크다. ' +
      '투자 손실, 사업 변동, 남명에서는 배우자와의 갈등이 심화된다.',
  },
  {
    type: 'CHUNG',
    sipsins: ['JEONG_IN', 'PYEON_IN'],
    description:
      '인성충(印星沖) — 인성의 지지가 충돌하면 학업 중단, 이사, 어머니와의 이별 등이 우려된다. ' +
      '정신적 안정이 흔들리며 문서·계약 관련 문제가 생긴다.',
  },
  {
    type: 'CHUNG',
    sipsins: ['PYEON_GWAN'],
    description:
      '칠살충(七殺沖) — 편관의 지지가 충돌하면 관재·사고·건강 문제가 부각된다. ' +
      '다만 신강 사주에서 기신인 칠살이 충거되면 오히려 길하다.',
  },

  // -- 형(刑)에 의한 변화 --
  {
    type: 'HYEONG',
    sipsins: ['JEONG_GWAN', 'PYEON_GWAN'],
    description:
      '관성형(官星刑) — 관성이 위치한 지지가 형(刑)을 당하면 ' +
      '법적 분쟁, 직장 문제, 권위의 추락이 우려된다. ' +
      '삼형(三刑)이 관성을 해치면 더욱 심각하다.',
  },
  {
    type: 'HYEONG',
    sipsins: ['JEONG_JAE', 'PYEON_JAE'],
    description:
      '재성형(財星刑) — 재성이 위치한 지지가 형을 당하면 ' +
      '재물 손실과 함께 건강 문제가 동반될 수 있다.',
  },
];
 ...
 * 십신 5그룹 생극 순환 순서
 *
 * 비겁(0) -> 식상(1) -> 재성(2) -> 관성(3) -> 인성(4) -> 비겁(0)
 *
 * 생(生) 관계: cycle[i] -> cycle[(i+1) % 5]
 * 극(剋) 관계: cycle[i] -> cycle[(i+2) % 5]
 *
 * 이 순환은 오행(五行) 상생·상극의 십신 버전이다.
 */
export const SIPSIN_GROUP_CYCLE: readonly SipsinGroup[] = [
  'BIGEOB',   // 0: 비겁 (비견·겁재)
  'SIKSANG',  // 1: 식상 (식신·상관)
  'JAE',      // 2: 재성 (편재·정재)
  'GWAN',     // 3: 관성 (편관·정관)
 ...
 * @param from - 기준 그룹
 * @param to - 대상 그룹
 * @returns 관계 문자열: 'SAENG' (생), 'GEUK' (극), 'DONG' (동, 같은 그룹),
 *          'PIGEUK' (피극, 극 당함), 'PISAENG' (피생, 생 받음)
 */
export function sipsinGroupRelation(
  from: SipsinGroup,
  to: SipsinGroup,
): 'SAENG' | 'GEUK' | 'DONG' | 'PIGEUK' | 'PISAENG' {
  const cycle = SIPSIN_GROUP_CYCLE;
  const fi = cycle.indexOf(from);
  const ti = cycle.indexOf(to);
  const diff = ((ti - fi) % 5 + 5) % 5;

  switch (diff) {
    case 0: return 'DONG';     // 같은 그룹
    case 1: return 'SAENG';    // from이 to를 생
    case 2: return 'GEUK';     // from이 to를 극
    case 3: return 'PIGEUK';   // from이 to에게 극 당함 (= to가 from을 극)
    case 4: return 'PISAENG';  // from이 to에게 생 받음 (= to가 from을 생)
    default: return 'DONG';
  }
}
