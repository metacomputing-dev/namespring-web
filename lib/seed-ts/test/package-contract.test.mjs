...
test('manifest exposes one ESM package entry and a clean prepack build', () => {
  ...
  assert.equal(
    ...
    'npm run test:database-asset-manifest && npm run check:fourframe-catalog && npm run test:sql-js-wasm-asset && npm run build',
  );
  assert.equal(
    manifest.scripts['test:database-asset-manifest'],
    'npm run check:database-asset-manifest && tsx test/database-asset-manifest-parity.test.ts',
  );
  assert.equal(
    manifest.scripts['test:contracts']
      .split(' && ')
      .includes('npm run test:database-asset-manifest'),
    true,
  );
