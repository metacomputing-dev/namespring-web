 ...
 *
 * sql.js + fetch() 를 Node.js에서 사용하기 위해 환경을 패치합니다.
 */
...
globalThis.fetch = async (url, options) => {
  ...
  // For WASM URL, try local sql.js node_modules
  if (typeof url === 'string' && url.includes('sql-wasm.wasm')) {
    const wasmPath = path.resolve(SEED_TS_DIR, 'node_modules/sql.js/dist/sql-wasm.wasm');
...
// ── Import seed-ts ──
const { SeedEngine } = await import('../dist/engine.js');

const birth = { year: 1986, month: 4, day: 19, hour: 5, minute: 45, gender: 'male' };
