...
/** Yongshin data extracted from saju-ts analysis */
...
export function computeRecommendationScore(
  ...
  for (const rec of yongshin.recommendations) {
    ...
    const recConfidence = Number.isFinite(rec.confidence)
      ? clamp(rec.confidence, 0, 1)
      : SAJU_DEFAULT_RECOMMENDATION_CONFIDENCE;
    const typeWeight = YONGSHIN_TYPE_WEIGHT[rec.type] ?? YONGSHIN_TYPE_WEIGHT_DEFAULT;
    ...
    totalWeight += weight;
    if (CONTEXTUAL_YONGSHIN_TYPES.has(rec.type)) {
