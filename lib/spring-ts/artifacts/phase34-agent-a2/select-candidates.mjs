// Phase 34 Agent A2 -- find 4p fragments in owned scope, with ct ∈ [1..3] in 4p cells
// Excludes fragments already lifted to 5p by P26-A3 / P27-A3 / P30-A1 (21 fragments).
// Goal: 5p band 267 → ~280 (+13). Math: pick ct mix summing to exactly 13.
...
// P26-A3 (10) + P27-A3 (10) + P30-A1 (7) = 27 fragments lifted to 5p across phases.
// (Note: P26-A3 + P27-A3 overlap on 6 fragments — they both lifted 4p->5p in two
// passes; the union is 14 unique. Plus P30-A1 wealth.{thisMonth,thisWeek,today} 7 = 21 unique.)
const ALREADY_5P_LIFTED = new Set([
  // P26-A3 (10 frags, 4p->5p; insert-P4 mode on existing 4p fragments)
...
fs.writeFileSync(
  'artifacts/phase34-agent-a2/candidates-4p.json',
  JSON.stringify(
    candidates.map((c) => ({
      fragmentId: c.fid,
      ct: c.ct,
