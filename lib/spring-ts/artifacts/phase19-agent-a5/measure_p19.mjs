/**
 * artifacts/phase19-agent-a5/measure_p19.mjs
 *
 * Phase 19 Agent A5 audit measurement.
 *
 * Successor to `measure_p18_a5.mjs`. Same shape (paragraph histograms +
 * post-processor leak monitors + cluster monitors + per-fixture hook
 * distribution), with P19 acceptance gate extensions:
 *
 *   1. Non-_coverage 연결X scan across `data/narrative/<cat>/**` (8
 *      particle forms: 을/이/로/의/에/과/도/만 per
 *      PLAN_PHASE19_COMPOUND_HARDENING.md §P19-A1).
 *   2. Forbidden-scope 연결X scan in `_metaphor/*.json` and
 *      `_glossary/*.json` (counted separately because A1 owns these).
 *   3. Doubled `결이` cluster monitor — paragraphs/headlines containing
 *      `결이` ≥2× (P18-A4's metric, distinct from `결` ≥3 density).
 *   4. Sub-3-paragraph standard cell breakdown (P18-A3 baseline 107).
 *   5. brief.hook dormant slot count (per-fragmentId view from
 *      `measure_p18_a2.mjs`, plus a per-text view).
 *
 * Read-only on data + samples. Pure JSON parse + aggregate. Does NOT
 * regenerate samples and does NOT touch any forbidden surface.
 *
 * Usage: node artifacts/phase19-agent-a5/measure_p19.mjs [out.json]
 */
...
function scanYeongyeolDir(dir) {
  if (!fs.existsSync(dir)) {
    return { total: 0, perFile: {}, vulnTotal: 0, vulnPerFile: {} };
  }
  const out = { total: 0, perFile: {}, vulnTotal: 0, vulnPerFile: {} };
  const files = fs
    .readdirSync(dir)
