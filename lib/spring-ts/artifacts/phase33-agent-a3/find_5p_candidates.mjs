 ...
 *
 * P33-A3 — Walk every expert.fragments.json. For each 5-paragraph
 * fragment, count fire frequency across the sample fixtures.
 * Print top candidates by fire count.
 *
 * Adapted from artifacts/phase32-agent-a3/find_5p_candidates.mjs
 * (5p→6p target, but exclude prior-lifted set rather than mark).
 *
 * Per P33-A3 task brief:
 *   "P28-A2/P30-A4/P32-A3 lift fragments 무수정"
 * — therefore we EXCLUDE those 30 fragments from candidates.
 */
...
// 30 fragments lifted by P28-A2 + P30-A4 + P32-A3. EXCLUDED from selection.
const PREVIOUSLY_LIFTED = new Set([
  ...
  // P32-A3 (5p→6p, 10) -- exclude (already 6p anyway)
...
console.log('\nTop 50 5-paragraph expert fragments by fire count (post-exclusion):\n');
...
fs.writeFileSync(
  path.join(__dirname, 'candidates-2026-05-08.json'),
  JSON.stringify(out, null, 2) + '\n',
);
console.log('\nWrote candidates-2026-05-08.json');
