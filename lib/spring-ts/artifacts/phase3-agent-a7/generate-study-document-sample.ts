/**
 * artifacts/phase3-agent-a7/generate-study-document-sample.ts
 *
 * Regression sample generator for Phase 3 Agent A7 (study_document polish).
 * Reproduces the 1986-04-19 male choi-seongsoo fixture and writes the
 * `tieredMatrix.periods.life.study_document` cell (and other periods) to a
 * caller-specified JSON file (after-sample.json or before-sample.json).
 *
 * Run via:
 *   npx tsx artifacts/phase3-agent-a7/generate-study-document-sample.ts <output-file>
 */
...
const out: Record<string, any> = {
  ...
  scope: 'tieredMatrix.periods.<period>.{study_document or categoryFortunes[study_document]}',
  cells: {} as Record<string, any>,
};

function findStudyDoc(node: any): any {
  if (!node || typeof node !== 'object') return null;
  // tiered matrix structure: byCategory.study_document
  if (node.byCategory?.study_document) return node.byCategory.study_document;
  if (node.study_document) return node.study_document;
  return null;
}

for (const period of ['life', 'today', 'thisWeek', 'thisMonth', 'thisYear']) {
  out.cells[period] = findStudyDoc(periods?.[period]);
}
