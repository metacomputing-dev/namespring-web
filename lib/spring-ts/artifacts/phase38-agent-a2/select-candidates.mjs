// Phase 38 Agent A2 -- 4p->5p candidate finder.
//
...
//
// Output: prints 4p fragments with their ct (firing count), bucketed
// by ct=1 / ct=2 / ct=3+ for combo planning. P37-A2 saw ct=1 pool
// nearly exhausted so P38-A2 must mix ct values to hit +10 cells.
//
// ALREADY_5P set extended with P37-A2's 10 ids (60 + 10 = 70 total),
// inherited from P37-A2 select-candidates and the P37-A2 commit body.
//
// Run from lib/spring-ts root:
//   `node artifacts/phase38-agent-a2/select-candidates.mjs`
...
// Already-5p lifted fragments across 7 prior lift phases.
// Excluded structurally because their paragraph count is now 5+.
// 70 ids total (P26-A3 10 + P27-A3 10 + P30-A1 7 + P34-A2 13 + P35-A3
// 10 + P36-A2 10 + P37-A2 10 = 70). Many entries are redundant since
// the 4p paragraph-count filter does the real exclusion regardless of
// explicit set membership; the explicit set is documentation+audit aid.
...
for (const c of buckets[1]) console.log(`  ${c.id}`);
console.log('--- ct=2 candidates ---');
for (const c of buckets[2]) console.log(`  ${c.id}`);
console.log('--- ct=3 candidates ---');
for (const c of buckets[3]) console.log(`  ${c.id}`);
console.log('--- ct>=4 candidates ---');
for (const c of buckets['4+']) console.log(`  ${c.id}  (ct=${c.ct})`);
