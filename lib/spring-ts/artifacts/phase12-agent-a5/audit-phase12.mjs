...
// ---------- (4) Depth inversion (brief vs standard semantic overlap) ----------
//
// P21-A2 refresh (2026-05-07): replaces the prior length-only comparator.
// The original detector flagged any cell where brief.headline char count
// exceeded standard.para0 char count. With both surfaces tightly bounded
// (brief 28-40 chars, standard first-sentence 30-45 chars) length deltas
// of 1-8 chars produced 113 hits across genuinely-different prose — pure
// noise.
//
// The semantic intent (carried over from P5/P6/P7-A1 depthCoherence) is
// to flag cells where the brief tier and the first sentence of the
// standard tier carry essentially the same wording, i.e. standard adds
// no depth over brief. The refreshed detector measures Levenshtein
// similarity between particle-stripped, whitespace-normalized forms and
// flags pairs at >= 0.75 ratio, matching the P5 distance threshold.

const DEPTH_INVERSION_PARTICLE_RE = /(은|는|이|가|을|를|의|에|에서|와|과|도|만|으로|로|요|예요|이에요|이다|다)$/;
...
function normalizeForOverlap(text) {
  ...
  for (let i = 0; i < 4; i += 1) {
    const before = s;
    s = s.replace(DEPTH_INVERSION_PARTICLE_RE, '');
    if (s === before) break;
  }
  return s;
}

const DEPTH_INVERSION_SIMILARITY_THRESHOLD = 0.75;
const DEPTH_INVERSION_MIN_LEN = 6;
...
function detectDepthInversion(records, fixtureName) {
  ...
  for (const [key, items] of groups) {
    ...
    if (a.length < DEPTH_INVERSION_MIN_LEN || b.length < DEPTH_INVERSION_MIN_LEN) continue;
    const dist = levenshtein(a, b);
    const maxLen = Math.max(a.length, b.length);
    const similarity = 1 - dist / maxLen;
    if (similarity < DEPTH_INVERSION_SIMILARITY_THRESHOLD) continue;
    hits.push({
      cellKey: key,
