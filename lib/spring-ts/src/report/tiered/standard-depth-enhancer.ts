  ...
  const sourceVariantValue = value
    ...
    .replace(
      ...
      pickVariant(ctx, 'sourceCareerMidlifeTrust', [
        '일과 사람 사이의 균형은 이 시기의 중요한 자산이에요. 한쪽 책임만 크게 붙잡기보다 내가 맡을 일과 나눌 일을 구분하면 다음 변화 앞에서도 덜 흔들려요.',
        ...
        '일과 관계의 균형을 자산으로 보아야 해요. 성과만 밀어붙이면 주변과의 호흡이 흔들릴 수 있으니, 작은 결정일수록 누구와 나눌지 먼저 생각해 보세요.',
      ]),
    )
    .replace(
...
function softenPublicTone(value: string): string {
  return normalizeRenderedText(value)
    ...
    .replace(/사주에 가장 필요한 기운이 물\(水\)인 사람의 인연 자리는 큰 사건의 단정보다, 천천히 스며드는 다정함이 모양을 잡는 흐름으로 풀려요/g, '관계에서는 큰 사건보다 천천히 쌓이는 다정함이 더 오래 남을 수 있어요')
    .replace(/사주에 가장 필요한 기운이 쇠\(金\)인 사람의 인연 자리는 한 번에 빠르게 만들어지기보다, 잘 다듬은 도구처럼 천천히 형태를 잡는 모양이에요/g, '관계에서는 한 번에 빠르게 가까워지기보다, 서로의 기준을 천천히 맞추며 신뢰를 쌓는 모습이 잘 어울려요')
