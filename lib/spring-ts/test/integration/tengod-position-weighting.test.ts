 ...
 *
 * Synthetic scorer-only fixtures for PR-5.1. These keep aggregate
 * `groupCounts` fixed while moving one ten-god signal across source layers
 * and pillar positions, so the current positional weighting behavior is
...
function score(id: FixtureId, mode: 'simple_count' | 'positional_weighted') {
...
console.log('PR-5.1 ten-god positional weighting synthetic fixtures\n');
...
for (const id of ids) {
  console.log(`${id.padEnd(12)} simple=${simpleRows[id].score.toFixed(6)} positional=${positionalRows[id].score.toFixed(6)}`);
}
