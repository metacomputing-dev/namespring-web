...
const legacyOnlySummary = extractSaju(legacyOnlyRawOutput as any);
...
check('unavailable boundary stays null without a fabricated term id',
...
const explicitNullSummary = extractSaju({
  ...rawOutput,
  daeunInfo: {
    ...rawOutput.daeunInfo,
    boundaryTermId: null,
...
check('explicit null boundaryTermId is authoritative over a stale legacy alias',
  explicitNullSummary.daeunInfo?.boundaryTermId === null &&
  explicitNullSummary.daeunInfo?.boundaryMode === '');
