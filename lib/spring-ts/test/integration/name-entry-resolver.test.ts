...

{
  ...
  const engine = new SpringEngine() as any;
  engine.hanjaRepo = {
    findByHanja: async () => ineligible,
    findByHangul: async () => [ineligible],
    findSurnamesByHangul: async () => [],
  };

  await assert.rejects(
    engine.resolveEntries([{ hangul: '\uAE40', hanja: '\u91D1' }], { isSurname: true }),
    (error: unknown) => {
      assert.ok(error instanceof NameEntryResolutionError);
      assert.equal(error.reason, 'explicit_hanja_not_surname_eligible');
      assert.equal(error.role, 'surname');
      assert.equal(error.characterIndex, 0);
      assert.equal(error.message.includes('\uAE40'), false);
      assert.equal(error.message.includes('\u91D1'), false);
      return true;
    },
    'a given-name row must never be promoted to surname authority',
  );
}

{
  const ineligibleFirstRow = fakeEntry({
    id: 11,
    hangul: '\uAE40',
    hanja: '\u91D1',
    is_surname: false,
  ...
  const engine = new SpringEngine() as any;
  engine.hanjaRepo = {
    findByHanja: async () => ineligibleFirstRow,
    findByHangul: async () => [ineligibleFirstRow],
  ...
  assert.equal(resolved[0].is_surname, true);
}
