...
check('day master slot style reinforced', prompt.includes('{{dayMasterName}}의 성분은') && prompt.includes('{{dayMasterName}}은 사용자 이름이 아니라'));
...
check('causal paragraph logic required', prompt.includes('어떤 상황을 가정했나?') && prompt.includes('왜 중요한가?') && prompt.includes('독자는 무엇을 하면 되는가?'));
check('hypothetical scene guidance required', prompt.includes('가정형을 기본으로 합니다') && prompt.includes('실제로 그 일을 이미 했다고 단정하지 마세요'));
