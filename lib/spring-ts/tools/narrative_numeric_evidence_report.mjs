...
function parseArgs(argv) {
  const args = {
    ...
    maxLowUsageAvailablePathThreshold: null,
  ...
  for (const arg of argv) {
    ...
    } else if (arg.startsWith('--max-low-usage-available-paths=')) {
      const value = Number(arg.slice('--max-low-usage-available-paths='.length));
      if (Number.isInteger(value) && value >= 0) args.maxLowUsageAvailablePathThreshold = value;
...
function buildNumericEvidenceReport(options = {}) {
  ...
  const maxLowUsageAvailablePathThreshold = options.maxLowUsageAvailablePathThreshold ?? null;
  ...
  const lowUsageAvailablePaths = AVAILABLE_NUMERIC_PATHS
    .map((expression) => ({
      expression,
      count: expressionCounts.get(expression) ?? 0,
      requiredCount: minExpressionUsage,
      deficit: Math.max(0, minExpressionUsage - (expressionCounts.get(expression) ?? 0)),
      thinUsageGated: THIN_USAGE_GATED_PATHS.includes(expression),
    }))
    .filter((row) => row.count > 0 && row.count < minExpressionUsage)
    .sort((a, b) => b.deficit - a.deficit || a.count - b.count || a.expression.localeCompare(b.expression));
  ...
  const lowUsageAvailablePathExcessToThreshold = maxLowUsageAvailablePathThreshold === null
    ? 0
    : Math.max(0, lowUsageAvailablePaths.length - maxLowUsageAvailablePathThreshold);
  ...
  return {
    ...
    maxLowUsageAvailablePathThreshold,
    ...
    totals: {
      ...
      lowUsageAvailablePathCount: lowUsageAvailablePaths.length,
      lowUsageAvailablePathDeficit: lowUsageAvailablePaths.reduce((sum, row) => sum + row.deficit, 0),
      ...
      lowUsageAvailablePathExcessToThreshold,
...
function getThresholdFailures(report) {
  ...
  if ((report.totals?.lowUsageAvailablePathExcessToThreshold ?? 0) > 0) {
    failures.push(
      `low-usage available numeric paths ${report.totals.lowUsageAvailablePathCount}/${report.maxLowUsageAvailablePathThreshold}`,
    );
  }
...
function renderHuman(report) {
  ...
  lines.push(`Low-usage available numeric paths (<${report.minExpressionUsageThreshold}): ${report.totals.lowUsageAvailablePathCount}`);
  ...
  if (report.lowUsageAvailablePaths.length > 0) {
    lines.push('');
    lines.push('Low-usage available numeric paths:');
    for (const row of report.lowUsageAvailablePaths) {
      lines.push(`- ${row.expression}: count=${row.count}, deficit=${row.deficit}, thinGate=${row.thinUsageGated}`);
    }
  }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const report = buildNumericEvidenceReport({
    ...
    maxLowUsageAvailablePathThreshold: args.maxLowUsageAvailablePathThreshold,
    maxExpertBundlesWithoutNumericalEvidenceThreshold:
