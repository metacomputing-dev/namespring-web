...
function parseArgs(argv) {
  ...
  for (const arg of argv) {
    ...
    } else if (arg.startsWith('--max-top-thin=')) {
      const value = Number(arg.slice('--max-top-thin='.length));
      if (Number.isInteger(value) && value >= 0) args.maxTopThin = value;
...
function buildPairReport(options = {}) {
  ...
  const maxTopThin = options.maxTopThin ?? 20;
  ...
  const pairReports = pairs.map(([leftField, rightField]) => {
    ...
    const pairKey = `${leftField}:${rightField}`;
    const allThinCombinations = thinCombinations.map((combo) => ({
      pairKey,
      fields: [leftField, rightField],
      values: {
        [leftField]: combo[leftField],
        [rightField]: combo[rightField],
      },
      authoredFragments: combo.authoredFragments,
      requiredAuthoredFragments: combo.requiredAuthoredFragments,
  ...
  const thinCombinationCount = pairReports.reduce((sum, pair) => sum + pair.thinCombinationCount, 0);
  const thinCombinationDeficit = pairReports.reduce((sum, pair) => sum + pair.authoredDeficitToThreshold, 0);
  const missingCombinationCount = pairReports.reduce((sum, pair) => sum + pair.missingCombinationCount, 0);
  ...
  const topThinCombinations = pairReports
    .flatMap((pair) => pair.allThinCombinations)
    .sort((a, b) =>
      b.deficit - a.deficit ||
      a.authoredFragments - b.authoredFragments ||
    ...
    .slice(0, maxTopThin);
  const publicPairReports = pairReports.map(({ allThinCombinations, ...pair }) => pair);
  return {
...
function getThresholdFailures(report) {
  ...
  if ((report.totals?.missingCombinationExcessToThreshold ?? 0) > 0) {
    failures.push(
      `missing pair combinations ${report.totals.missingCombinationCount}/${report.maxMissingCombinationThreshold}`,
    );
  }
  if ((report.totals?.thinCombinationExcessToThreshold ?? 0) > 0) {
    failures.push(
      `thin pair combinations ${report.totals.thinCombinationCount}/${report.maxThinCombinationThreshold}`,
    );
  }
  return failures;
}
...
function renderHuman(report) {
  ...
  lines.push(`Thin pair combinations (<${report.minAuthoredThreshold} authored): ${report.totals.thinCombinationCount}, deficit=${report.totals.thinCombinationDeficit}`);
  lines.push('');
  if (report.topThinCombinations.length > 0) {
    lines.push('Top thin combinations:');
    for (const combo of report.topThinCombinations) {
      const valueText = combo.fields
        .map((field) => `${field}=${combo.values[field]}`)
        .join(', ');
      lines.push(`- ${combo.pairKey}: ${valueText}; authored=${combo.authoredFragments}, deficit=${combo.deficit}`);
    }
    lines.push('');
  }
  for (const pair of report.pairs) {
    lines.push(`${pair.key}: ${pair.coveredCombinationCount}/${pair.expectedCombinationCount} covered (${pair.coverageRatio})`);
    lines.push(`  authored pair hits: ${pair.authoredFragmentCount}; thin=${pair.thinCombinationCount}; deficit=${pair.authoredDeficitToThreshold}`);
    ...
    if (pair.thinCombinations.length > 0) {
      lines.push('  top thin:');
      for (const combo of pair.thinCombinations) {
        const [leftField, rightField] = pair.fields;
        lines.push(`  - ${leftField}=${combo[leftField]}, ${rightField}=${combo[rightField]}: authored=${combo.authoredFragments}, deficit=${combo.deficit}`);
      }
    }
  }
  return lines.join('\n');
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (thresholdFailures.length > 0) {
    console.error(`Narrative axis-pair thresholds failed:\n- ${thresholdFailures.join('\n- ')}`);
    process.exit(1);
  }
}

export { buildPairReport, getThresholdFailures, renderHuman };
