...
const REPORT_SCHEMA_VERSION = 'spring-ts.reference-authority-intake-report.v1';
...
function validateCase(record, filePath) {
  ...
  if (sourceTierOk) {
    const rank = parseTierRank(record.sourceTier);
    const eligible = record.sourceTier.authorityTruthEligible === true;
    if (typeof record.sourceTier.authorityTruthEligible !== 'boolean') {
      addViolation(
        violations,
        filePath,
        'authority_truth_flag_missing',
        'sourceTier.authorityTruthEligible',
        'sourceTier.authorityTruthEligible must be boolean',
      );
    }
    if (eligible) {
      if (rank === null || rank < MIN_AUTHORITY_TRUTH_TIER) {
        addViolation(
          violations,
          filePath,
          'low_tier_authority_truth',
          'sourceTier.tier',
          `authority-truth records require T${MIN_AUTHORITY_TRUTH_TIER}+ source tier`,
          { tier: record.sourceTier.tier ?? null },
        );
      }
      if (record.sourceTier.sourceType === 'training_derived') {
        addViolation(
          violations,
          filePath,
          'training_derived_authority_truth',
          'sourceTier.sourceType',
          'training-derived records cannot be authority truth',
        );
      }
      if (sourceOk && isUnresolved(record.source.page)) {
        addViolation(
          violations,
          filePath,
...
function validateDirectory(dir, allowEmpty) {
  ...
  for (const filePath of files) {
    try {
      ...
      if (record?.sourceTier?.authorityTruthEligible === true) authorityTruthEligibleCaseCount += 1;
      caseSummaries.push({
        file: relPath(filePath),
