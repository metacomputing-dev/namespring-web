...
test('T5 KOSIS statistics are domain-pinned naming-data authority only', () => {
  const record = {
    id: 'kosis-compound-surname-minimum',
    sourceTier: tier({
      tier: 'T5_OFFICIAL',
      sourceType: 'official_kosis_statistics',
      sourceUrl: 'https://kosis.kr/statisticsList/example',
  ...
  assert.equal(isAuthorityTruthEligible(record, {
    root: SPRING_TS_ROOT,
    requiredScope: AUTHORITY_SCOPES.NAMING_DATA,
  }), true);
  assert.equal(isAuthorityTruthEligible(record, {
    root: SPRING_TS_ROOT,
    requiredScope: AUTHORITY_SCOPES.SAJU_DOCTRINE,
  ...
  const spoofed = structuredClone(record);
  spoofed.sourceTier.sourceUrl = 'https://example.test/kosis-mirror';
  assert.ok(validateSourceTierRecord(spoofed, { root: SPRING_TS_ROOT })
    .some((row) => row.code === 'invalid_official_source_domain'));
});

test('T5 eFamily lookup is domain-pinned legal-name authority only', () => {
  const record = {
    id: 'official-hanja-lookup',
    sourceTier: tier({
      tier: 'T5_OFFICIAL',
      sourceType: 'official_court_lookup',
      sourceUrl: 'https://efamily.scourt.go.kr/cs/CsBltnWrtList.do?bltnbordId=0000010',
      authorityReview: undefined,
    }),
  };
  assert.deepEqual(validateSourceTierRecord(record, { root: SPRING_TS_ROOT }), []);
  assert.equal(isAuthorityTruthEligible(record, {
    root: SPRING_TS_ROOT,
    requiredScope: AUTHORITY_SCOPES.LEGAL_NAME_DATA,
  }), true);
  assert.equal(isAuthorityTruthEligible(record, {
    root: SPRING_TS_ROOT,
    requiredScope: AUTHORITY_SCOPES.NAMING_SCORE_CALIBRATION,
  ...
  const spoofed = structuredClone(record);
  spoofed.sourceTier.sourceUrl = 'https://example.test/hanja-mirror';
  assert.ok(validateSourceTierRecord(spoofed, { root: SPRING_TS_ROOT })
    .some((row) => row.code === 'invalid_official_source_domain'));
});
