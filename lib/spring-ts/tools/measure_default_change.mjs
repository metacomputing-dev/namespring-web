...
export function runMeasure(baselineSnapshot, branchSnapshot, approvalManifest, refs = {}) {
  ...
  const approval = resolveDiffApproval(exactDiffs, approvalManifest);

  let rawOverall;
  if (totals.regression > 0 || totals.missing > 0) rawOverall = 'REGRESSION';
  else if (totals.reviewRequired > 0 || exactDiffs.length > 0) rawOverall = 'REVIEW_REQUIRED';
  else rawOverall = 'UNCHANGED';
  let overall = rawOverall;
  if (rawOverall !== 'REGRESSION') {
    if (['RELEASE_BLOCKED', 'MANIFEST_INVALID'].includes(approval.status)) {
      overall = 'RELEASE_BLOCKED';
    } else if (rawOverall === 'REVIEW_REQUIRED' && approval.status === 'APPROVED') {
      overall = 'APPROVED_CHANGE';
    }
  }
...
export function runCli(argv = process.argv) {
  ...
  const report = {
    ...runMeasure(baseline, branch, manifest, args),
    approvalManifestPath,
  };
  if (args.json) console.log(JSON.stringify(report, null, 2));
  else console.log(renderHuman(report, args.verbose));
  return ['UNCHANGED', 'APPROVED_CHANGE'].includes(report.overall) ? 0 : 1;
}
