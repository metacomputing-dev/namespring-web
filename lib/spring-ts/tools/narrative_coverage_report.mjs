...
function parseArgs(argv) {
  const args = { json: false, minAuthored: 5, minExpertNumericalEvidence: 1 };
...
function buildReport(options = {}) {
  ...
  for (const file of bundles) {
    ...
    for (const fragment of bundle.fragments ?? []) {
      ...
      if (fragment?.sourceTier?.authorityTruthEligible === true) {
  ...
  const thinAxisFieldSummary = [...thinAxisValues.reduce((map, row) => {
