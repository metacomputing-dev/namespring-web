...
function readSource() {
  ...
  for (const [index, entry] of document.entries.entries()) {
    ...
    if (!isSingleUnicodeScalar(glyph)) {
      fail(`entry ${index} must contain exactly one Unicode scalar value`);
    }
    ...
    if (!Array.isArray(entry.readings)
      ...
      fail(`entry ${index} has invalid or duplicate designated readings`);
    }
    const sortedReadings = [...entry.readings].sort(compareCodeUnits);
    if (sortedReadings.length > 4) {
      fail(`entry ${index} exceeds the compact reading-count contract`);
    }
    readingRows.push(sortedReadings.join(''));
    readingCounts.push(sortedReadings.length);
    glyphs.push(glyph);
  }

  const joinedGlyphs = glyphs.join('');
  const joinedReadings = readingRows.join('');
  return {
    document,
    joinedGlyphs,
    joinedReadings,
...
function buildOutput() {
  const { document, joinedGlyphs, joinedReadings, readingCounts } = readSource();
  const encodedReadingCounts = Buffer.from(readingCounts);
  return `${JSON.stringify({
    schemaVersion: '1.0.0-glyph-registry',
    sourceSchemaVersion: document.schemaVersion,
    // Hash canonical parsed JSON so checkout EOL policy cannot change output.
    sourceDocumentSha256: sha256(JSON.stringify(document)),
    glyphsSha256: sha256(joinedGlyphs),
    count: Array.from(joinedGlyphs).length,
    glyphs: joinedGlyphs,
    readingPairCount: joinedReadings.length,
    readingsSha256: sha256(joinedReadings),
    readingCountsSha256: sha256(encodedReadingCounts),
    readingCountsEncoding: 'uint8-base64',
    readingCountsBase64: encodedReadingCounts.toString('base64'),
    readings: joinedReadings,
  })}\n`;
}
