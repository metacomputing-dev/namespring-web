...
function validateSourceTierObject(sourceTier, filePath, sourceTierPath = 'sourceTier') {
  ...
  if (!sourceTier || typeof sourceTier !== 'object' || Array.isArray(sourceTier)) {
    return [{
      file,
      sourceTierPath,
      code: 'missing_sourceTier',
      message: `${sourceTierPath} object is required`,
    }];
  }
  ...
  for (const field of SOURCE_TIER_REQUIRED_FIELDS) {
    if (!(field in sourceTier)) {
      violations.push({
        file,
        sourceTierPath,
        code: 'missing_sourceTier_field',
        field,
        message: `${sourceTierPath}.${field} is required`,
      });
    }
  }
  ...
  if (rank === null) {
    violations.push({
      file,
      sourceTierPath,
      code: 'invalid_sourceTier_tier',
      message: `invalid tier: ${sourceTier.tier}`,
    });
  }
  if (typeof sourceTier.authorityTruthEligible !== 'boolean') {
    violations.push({
      file,
      sourceTierPath,
      code: 'invalid_authorityTruthEligible',
      message: `${sourceTierPath}.authorityTruthEligible must be boolean`,
    });
  }
...
function collectSourceTierRefs(data) {
  const refs = [{ sourceTier: data?.sourceTier, sourceTierPath: 'sourceTier' }];
  if (Array.isArray(data?.sources)) {
    data.sources.forEach((source, index) => {
      refs.push({
