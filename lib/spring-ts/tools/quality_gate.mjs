...
function evaluateD1(fixture, snapshotResult, authorityCase, oracleCase) {
  ...
  const categoricalA = [
    { field: 'sajuReport.gyeokgukType', actual: sajuActual.gyeokgukType, expected: authorityCase?.expected?.gyeokguk },
    { field: 'sajuReport.yongshinElement', actual: sajuActual.yongshinElement, expected: authorityCase?.expected?.yongshinElement },
    { field: 'sajuReport.strengthLevel', actual: sajuActual.strengthLevel, expected: authorityCase?.expected?.strengthLevel },
  ];
  for (const c of categoricalA) {
    ...
    if (isAllowedField(allowed, c.field)) {
      ...
      checks.push({ ...c, pass: c.actual === c.expected });
    }
  }

  // categorical fields from oracle (B) — same fields as oracle ground truth
  if (oracleCase?.expected) {
    const categoricalB = [
      { field: 'sajuReport.gyeokgukType', actual: sajuActual.gyeokgukType, expected: oracleCase.expected.gyeokgukType },
      { field: 'sajuReport.yongshinElement', actual: sajuActual.yongshinElement, expected: oracleCase.expected.yongshinElement },
      { field: 'sajuReport.strengthLevel', actual: sajuActual.strengthLevel, expected: oracleCase.expected.strengthLevel },
    ];
    for (const c of categoricalB) {
      ...
      if (isAllowedField(allowed, c.field)) {
        ...
        checks.push({ ...c, pass: c.actual === c.expected, source: 'oracle' });
      }
    }
  }

  // numerical: totalScore + scores.{hangul,hanja,fourFrame}
  const expectedNumerical = authorityCase?.expected?.scores || oracleCase?.expected?.scores;
  if (expectedNumerical) {
    const numericalChecks = [
      { field: 'namingReport.totalScore', actual: namingActual.totalScore, expected: authorityCase?.expected?.totalScore ?? oracleCase?.expected?.totalScore, tol: D1_TOTALSCORE_TOLERANCE },
  ...
  if (checks.length === 0) {
    return { dimension: 'D1', status: 'N/A', reason: 'no authority/oracle reference data for this fixture' };
  }
...
function inferSurfacedCardsFromSnapshot(snapshotResult) {
  ...
  // shinsal is not yet captured in baseline_snapshot.json; treat it as
  // "surfaced" iff the underlying saju engine populated shinsal entries
  // (PR-H-B surfaces them on SajuOutputSummary). Snapshot-only check
  // therefore can't disambiguate — when oracle expects 'shinsal' we
  // treat its presence as surfaced (saju-adapter PR-H-B always lifts
  // when source is non-empty); a future capture pass will add a direct
  // shinsal count to the snapshot.
  tokens.add('shinsal');
  return tokens;
}
...
function evaluateD5(fixture, snapshotResult, authorityCase, oracleCase) {
  ...
  if (authorityCase || oracleCase) {
    const d1 = evaluateD1(fixture, snapshotResult, authorityCase, oracleCase);
