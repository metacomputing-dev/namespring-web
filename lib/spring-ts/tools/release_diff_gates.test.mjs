...
test('release suite builds saju once and runs no-build engine gates', () => {
  ...
  assert.equal(
    packageJson.scripts['test:name-stat-summary'],
    'npm run typecheck:name-stat-summary && npx tsx --test test/integration/name-stat-summary-asset.test.ts test/integration/name-stat-summary-repository.test.ts',
  );
  const releaseCommands = releaseSuite.split('&&').map((command) => command.trim());
  const buildCommand = 'npm --prefix ../saju-ts run build';
  assert.equal(releaseCommands[0], buildCommand);
  assert.equal(
    releaseCommands.filter((command) => command === buildCommand).length,
    1,
  );
  for (const script of [
    'test:name-stat-summary',
    'test:gyeokguk-candidates:no-build',
    'test:saju-disabled',
    'test:saju-calculator-state',
    'test:tengod-position-weighting',
    'test:tengod-report-surface',
    'test:adaptive',
    'test:jonggyeok-authority:no-build',
    'test:adapter-yinyang:no-build',
    'test:adapter-richness-optins:no-build',
  ]) {
    assert.match(
      releaseSuite,
  ...
  for (const [standalone, noBuild, testCommand] of [
    [
      'test:gyeokguk-candidates',
      'test:gyeokguk-candidates:no-build',
      'npx tsx test/integration/gyeokguk-candidates-baseline.test.ts',
    ],
    [
      'test:jonggyeok-authority',
      'test:jonggyeok-authority:no-build',
      'npx tsx test/integration/jonggyeok-authority-scaffold.test.ts',
    ],
    [
      'test:adapter-yinyang',
      'test:adapter-yinyang:no-build',
      'npx tsx test/integration/adapter-yinyang.test.ts',
    ],
    [
      'test:adapter-richness-optins',
      'test:adapter-richness-optins:no-build',
      'npx tsx test/integration/adapter-richness-optins.test.ts',
    ],
  ]) {
    assert.equal(
      packageJson.scripts[standalone],
      `${buildCommand} && ${testCommand}`,
    );
    assert.equal(packageJson.scripts[noBuild], testCommand);
    assert.doesNotMatch(packageJson.scripts[noBuild], /saju-ts run build/u);
    assert.equal(releaseCommands.includes(`npm run ${standalone}`), false);
  }
});
