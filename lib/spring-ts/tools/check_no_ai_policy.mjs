...
function isRegularFileInsideRoot(root, candidate) {
  ...
  try {
    ...
    return Boolean(realRelative) &&
      ...
      fs.statSync(candidateRealPath).isFile();
  } catch {
    return false;
  }
}

function isPanelAdjudicatedAuthority(sourceTier, root) {
  ...
  const models = Array.isArray(adjudication.models)
    ? adjudication.models
      ...
      .map((m) => m.trim().toLocaleLowerCase('en-US'))
    : [];
  if (new Set(models).size < 2) return false; // distinct cross-model verification, not duplicated labels
  if (!(adjudication.adversarialVerification === true)) return false;
  if (!isRegularFileInsideRoot(root, adjudication.dossier)) return false;
...
function auditJsonFile(root, filePath, options) {
  ...
  for (const item of sourceTierRecords) {
    ...
    const panelAdjudicated = isPanelAdjudicatedAuthority(item.sourceTier, root);
