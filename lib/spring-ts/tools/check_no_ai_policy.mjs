...
function matchesAiSource(value) {
  ...
  return AI_SOURCE_PATTERNS.some((pattern) => pattern.test(value));
}
...
// 2026-07-10 policy v2: the only AI-derived class that may carry authority
// truth is the fully disclosed, owner-adjudicated panel record. Everything
// below must hold simultaneously — silence or partial disclosure keeps the
// original fail-closed rules in force. See NO_AI_POLICY.md "Panel-adjudicated
// exception" and SOURCE_TIER_POLICY.md for the governance rationale.
const PANEL_ADJUDICATED_SOURCE_TYPE = 'ai_panel_adjudicated_interpretation';

function hasApprovedReview(review) {
  return review?.status === 'approved' &&
    typeof review?.reviewedBy === 'string' && review.reviewedBy.trim().length > 0 &&
    /^\d{4}-\d{2}-\d{2}$/.test(String(review?.reviewedAt ?? ''));
}

function isPanelAdjudicatedAuthority(sourceTier) {
  if (sourceTier?.sourceType !== PANEL_ADJUDICATED_SOURCE_TYPE) return false;
  if (sourceTier?.aiGenerated !== true) return false; // disclosure is mandatory
  const adjudication = sourceTier?.panelAdjudication;
  if (!adjudication || typeof adjudication !== 'object') return false;
  const models = Array.isArray(adjudication.models)
    ? adjudication.models.filter((m) => typeof m === 'string' && m.trim().length > 0)
    : [];
  if (models.length < 2) return false; // cross-model verification, not single-model output
  if (!(adjudication.adversarialVerification === true)) return false;
  const dossier = adjudication.dossier;
  if (typeof dossier !== 'string' || dossier.trim().length === 0) return false;
  if (!fs.existsSync(path.resolve(SPRING_TS_ROOT, dossier))) return false; // evidence must exist in-repo
  return hasApprovedReview(sourceTier?.authorityReview);
}

function sourceTypeMarksAiProvenance(sourceType) {
  if (sourceType === PANEL_ADJUDICATED_SOURCE_TYPE) return true;
...
function auditJsonFile(root, filePath, options) {
  ...
  for (const item of sourceTierRecords) {
    ...
    const panelAdjudicated = isPanelAdjudicatedAuthority(item.sourceTier);
    if (authorityTruthEligible !== false && !panelAdjudicated) {
      violations.push({
        ...base,
        code: 'ai_authority_truth_eligible',
        message: 'AI-derived records must have sourceTier.authorityTruthEligible=false ' +
          '(only complete panel-adjudicated records — disclosed aiGenerated, 2+ models, ' +
          'adversarial verification, in-repo dossier, approved owner review — are exempt)',
      });
    }
    if (rank !== null && rank >= 3 && !panelAdjudicated) {
      violations.push({
        ...base,
