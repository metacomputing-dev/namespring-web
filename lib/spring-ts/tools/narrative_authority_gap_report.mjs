...
function renderHuman(report) {
  ...
  for (const row of report.byDepth) {
    lines.push(`- ${row.key}: zeroAuthority=${row.zeroAuthorityCellCount}/${row.cellCount}, authored=${row.authoredFragments}, authorityEvidence=${row.authorityTruthEligibleNumericalEvidenceRecords}`);
  }
  lines.push('');
