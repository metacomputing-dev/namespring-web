...
export function verifyManifestEvidenceAtRef({ repoRoot, evaluatedRef, evidence = [] } = {}) {
  ...
  return { valid: errors.length === 0, evaluatedCommit, evidencePaths: [...evidencePaths], errors };
}
...
export function verifyAttestationOnly({
  ...
  const evidenceVerification = verifyManifestEvidenceAtRef({
    repoRoot,
    evaluatedRef: evaluatedCommit,
    evidence,
  });
  errors.push(...evidenceVerification.errors);
  const evidencePaths = new Set(evidenceVerification.evidencePaths);
