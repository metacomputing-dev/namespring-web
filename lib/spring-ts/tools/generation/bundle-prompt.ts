...
7. **이름↔사주 이야기의 위치·비중을 편마다 달리하세요** — 모든 편의 마지막 문단이 이름 얘기로
...
1. **평문 tier(summary·body·tips·cautions)에 사주 용어 금지**: ${JARGON_BANNED} 등.
   오행의 우리말 이름(나무·불·흙·쇠·물)과 평문 형용사만 허용.
...
3. **모순 0**: ${cases.length}편 전부가 같은 사람 — 강약·격국·이름 방향이 편끼리 어긋나면 안 됩니다.
   ...
   ⚠ 강약 평문 형용사 **"${s.strengthPlain}"은 매 편의 평문(summary 또는 body)에 이 단어 그대로 최소
   1회 포함**하세요 — 검증기가 문자 그대로 대조하므로 유의어·의역("균형 잡힌", "치우치지 않는" 등)으로
   대체하면 그 편은 리젝됩니다.
...
## 분량 (편당 — 엄수)
- summary ≤60자 1문장 해요체. hook(선택) ≤24자 — 넘칠 것 같으면 생략.
- body 3~4문단, **각 문단 80~240자·2~5문장 — 마지막 문단 포함(한두 문장짜리 짧은 맺음말 문단 금지)**,
  전체 350~800자. 모든 문장 해요체(습니다/이다/명사 종결 금지).
- expert 1~2문단, 각 100~380자, #{태그} 2~6개.
