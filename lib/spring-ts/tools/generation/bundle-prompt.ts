...
2. **이름효과를 body 1문단 또는 3문단에 딱 1회, 비단정형으로 반영**(expert에만 두지 말 것). 예: "이름에서 보강되는 배움의 힘도 한 가지에 몰입하는 쪽과 잘 맞아요", "이름이 보태는 배움의 기운은 넓게 벌리기보다 하나를 붙드는 쪽으로 읽기 좋아요". ⚠ **이름효과가 나타나는 의미를 기간마다 다르게** 하라 — 오늘=한 번 앉아 시작한 공부를 그 자리에서 끝낸 느낌, 이번주=요일별 공부 흐름의 지속, 이번달=한 과목 마무리, 올해=상반기 방향 설정, 평생=오래 붙들어 축적. "하나에 집중"으로 15편을 수렴시키지 말 것. ⚠ **이름효과 문장 틀을 3~4종으로 로테이션**(비교형 "A보다 B일 때 ~" / 도치·조건형 "B일수록 ~ 또렷해져요" / 부정형 "A로는 잘 안 살고 B일 때 ~" / 명사구형 "이름이 더해 주는 든든함은 B에서 ~")하고, **문단 위치도 1·2·3문단으로 편마다 다르게** 둔다 — 같은 틀이 같은 자리에 반복되면 스탬핑처럼 읽힌다. ⚠ 틀뿐 아니라 **이름효과 주어 명사구도 편마다 바꿔라**(이름값 / 이름 덕에 붙는 뒷심 / 이름이 보태는 힘 / 이름이 더해 주는 든든함 등) — "이름이 받쳐 주는 배움의 뒷심" 같은 같은 구절을 4편 이상 쓰면 스탬핑으로 리젝된다. (nameEffect가 adverse/neutral이면 페어링 규칙을 우선하고 이 항목은 무시.)
3. **학업 이벤트를 기간마다 최소 1개** 구체적으로 넣어라(추상 '분량/주제' 대신): 시험(범위·일정), 자격증(과목), 강의(수강·복습), 발표·제출, 업무 학습. 기간 전략 단위 = 오늘: 한 번 앉아 끝낼 분량·오늘 끝낼 과제 / 이번주: 요일별 공부 흐름·주중 점검 / 이번달: 달 초 목표·중간 점검·말일 마감 / 올해: 상반기 방향·하반기에 성과로 잇기 / 평생: 타고난 학습 방식·장기 전략.
...
22. **쉽고 또렷하게 — 소설·비유체 금지(가독성 최우선, 유료 독자가 한 번에 이해해야 함)**:
    - ⚠ **학습을 은유로 포장하지 마라** — 공부를 걷기·여행·요리·농사 등으로 빗대지 말 것. 금지 예: "한 구간을 걷다/딴 데로 새다/완주하다", "한 바퀴 돌다", "여물다/영글다", "국 한 냄비 젓듯·식단대로 차리듯" 같은 은유·직유("~하듯"). 항목9의 팔레트 소재는 **구체적 공부 장면의 배경**으로만 쓰고, 공부를 그 소재에 빗대는 직유로 쓰지 마라.
    - ⚠ **외래어·모호어 금지(전 격국 공통)** — '세션'(→'한 번 앉아 하는 공부/한 자리'), '마일스톤'(→'중간 목표점'), '스텝'(→'단계'), '체크리스트'(→'점검 목록'), '커리큘럼'(→'짜인 과정'), '포트폴리오'(→'정리물/모음집'), '루틴'(→'습관'), '리듬' 남발 금지. 뜻이 바로 안 오는 말은 쉬운 우리말로. '완주'도 걷기 은유로 읽히니 '끝까지 마치기/끝내기'로.
    - ⚠ **추상 은유 오프닝 금지** — "~ 선이 몸에 맞게 다가와요", "배움의 저력이 뒤를 받쳐 줘요"처럼 무엇을 하라는지 모호한 문장으로 열지 말고, **구체적 공부 행동**으로 열어라.
    - ⚠ **한 문장은 한 번에 읽히게** — 만연체(한 문장에 여러 절을 겹겹이)·이중 비유 금지. 문장을 짧게 끊고 한 문장에 한 가지만 담아라.` : '';
