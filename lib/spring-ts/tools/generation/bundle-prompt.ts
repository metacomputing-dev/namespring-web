...
1. **body는 "공부 팁"이 아니라 "학업운 해석"이다. 3문단 구조로:**
   - 1문단: 이 시기에 공부가 **어떤 식으로 느껴질 수 있는지** + 어떤 공부 단위가 잘 맞는지. 형태 = [시기]+[공부 장면]+[느껴질 가능성]+[맞는 전략]. ⚠ 운세를 단정하지 말 것("학업운이 강하다/약하다·공부가 잘 된다·합격한다" 금지).
   - 2문단: 그 기간 단위에 맞는 **구체적 공부 사용법**(아래 학업 이벤트 활용).
   - 3문단: **무리하기 쉬운 지점과 조절법**(불안 조장·결과 단정 금지).
2. **이름효과를 body 1문단 또는 3문단에 딱 1회, 비단정형으로 반영**(expert에만 두지 말 것). 예: "이름에서 보강되는 배움의 힘도 한 가지에 몰입하는 쪽과 잘 맞아요", "이름이 보태는 배움의 기운은 넓게 벌리기보다 하나를 붙드는 쪽으로 읽기 좋아요". ⚠ **이름효과가 나타나는 의미를 기간마다 다르게** 하라 — 오늘=한 세션의 마무리감, 이번주=요일 리듬의 지속, 이번달=한 과목 완주, 올해=상반기 방향 설정, 평생=오래 붙들어 축적. "하나에 집중"으로 15편을 수렴시키지 말 것. (nameEffect가 adverse/neutral이면 페어링 규칙을 우선하고 이 항목은 무시.)
3. **학업 이벤트를 기간마다 최소 1개** 구체적으로 넣어라(추상 '분량/주제' 대신): 시험(범위·일정), 자격증(과목), 강의(수강·복습), 발표·제출, 업무 학습. 기간 전략 단위 = 오늘: 한 세션·오늘 끝낼 과제 / 이번주: 요일별 리듬·주중 점검 / 이번달: 달 초 목표·중간 점검·말일 마감 / 올해: 상반기 방향·하반기 성과화 / 평생: 타고난 학습 방식·장기 전략.
4. **추상어 상한(번들 전체 최대 2회): 흐름·기운·힘·결·바탕·잔잔.** "배움에 힘이 실려서" 같은 은유 오프닝을 반복하지 말고, 공부 장면에서 문장을 열어라.
5. **비겁 변별**: '같이 공부하는 사람(동료)' 모티프를 전 편에 깔지 말 것. 일부 편에만 쓰고, 나머지는 경쟁(같은 시험 준비자와 겨루기)·자기주도 점검·기록 비교 등 비겁의 다른 얼굴로 바꿔라.
6. **body 안전 표현(권장)**: ~하기 쉬운 편이에요 / ~로 느껴질 수 있어요 / ~와 잘 맞아요 / ~에 도움이 돼요 / ~쪽으로 가져가면 좋아요 / ~을 먼저 잡아두면 수월해요. **금지**: 학업운이 강해서/약해서, 반드시·무조건·확실히, 성공합니다·합격, 끝낼 수밖에·충분히 끝낼 수 있어요.
