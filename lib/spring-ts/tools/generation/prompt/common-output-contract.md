...
- body: 8-9문단. body[0]~body[3]은 첫 번째 완결 글, body[4]~body[7]은 두 번째 완결 글입니다. body[8]은 원칙적으로 작성하는 종합 마무리입니다. 단, summary 반복, 행동 목록, 새 소재 추가가 될 때만 생략하세요. 각 문단은 렌더링 기준 60-220자, 1-4문장. body 전체는 520-1500자. 길이를 채우기 위해 설명을 늘리지 말고, 문단이 이미 분명하면 짧게 끝내세요.
...
- {{dayMasterName}}은 사용자 이름이 아니라 `나무`, `불`, `흙`, `쇠`, `물` 중 하나로 치환되는 일간 오행명입니다. 평문에서는 되도록 `{{dayMasterName}}의 성분은 ...` 형태로 이어 쓰고, `{{dayMasterName}}은`, `{{dayMasterName}}이`처럼 바로 조사에 붙이지 마세요.
