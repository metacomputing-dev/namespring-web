...
## 분량(엄수 — 위반 시 리젝)
- summary ≤60자·1문장·해요체.
- **hook(선택): 반드시 ≤24자. 24자 넘길 것 같으면 아예 생략하세요.**
- body 3~4문단 각 80~240자, **모든 문장 반드시 '해요체'(요/죠/…예요/…이에요)로 끝맺음** — 격식체(습니다/이다)·명사종결 금지. 전체 350~800자.
- expert 1~2문단에 #{태그} 2~6개(모든 문장도 해요체).
- **슬롯 조사는 반드시 붙여서**: {{yongshinName:을를}} 형태. {{을를}} 처럼 조사만 슬롯으로 쓰지 마세요.
- livingTips 2~3(각 ≤30) / cautions 1~2(각 ≤44).
