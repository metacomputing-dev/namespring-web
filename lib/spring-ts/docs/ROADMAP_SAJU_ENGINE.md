...
>
> 현재 리뷰 단위: 누적 PR #653을 대체한 스택 PR #654~#678(25개). 21개는 Ready이고 #662·#665·#668·#671 4개는 Draft다. 진행 상태는 §9 표에 커밋 해시와 함께 갱신한다.
>
> **2026-07-14 merge-readiness 정정:** Ready는 독립 리뷰 시작 상태이고 merge 승인이 아니다. exact-tip 결함이 없고 회귀·한계가 공개된 21개 PR은 Ready이며, 실제 코드·CI P1이 남은 4개만 Draft다.
> 현재
...
| # | 항목 | 내용 | 완료 기준 |
|---|---|---|---|
| P0-1 | 스택 review와 merge gate 분리 | 완결된 diff·신규 P0/P1 없음·exact-tip 회귀·한계 공개를 충족하면 Ready로 전환한다. fingerprint·명리 승인·실제 CI·authority/signoff는 merge 차단선이며 composite 실패를 baseline 갱신으로 숨기지 않는다 | 각 PR의 base/head·Ready 근거·merge 차단선 §9 기록 |
...
|---|---|---|---|---|
| P0-1 스택 PR | 🟢 21개 Ready / 🔶 4개 Draft | PR #654~#678 | 2026-07-14 | 25개 base/head 스택을 병렬 재감사해 #662·#665·#668·#671만 Draft로 유지한다. #662는 월지손상 이중감점·기반 감쇠 층위 P1, #665는 exact-tip CI Seed 설치 누락, #668은 좌표 coercion, #671은 품질 증거 내부 모순이 원인이다. 후자의 세 결함은 후속 스택에서 해소됐지만 해당 historical tip에는 남아 backport 중이다. #676은 코드 P1=0이고 Ready/merge 문서 정합을 닫아 Ready로 전환한다. 두 global P1은 명리·영향 merge gate로 유지한다. Ready PR도 fingerprint·명리·Actions·authority/signoff 전 merge하지 않으며 frontend source diff는 0이다. |
...
| Stack25 Seed 공개 질의·오류 계약 | ✅ 코드·통합 회귀 통과 / Ready | d0b367581, 6f6b88053, PR #678 | 2026-07-14 | 공개 repository 질의·오류·Energy 입력을 fail-closed로 고정했고 유효 11,110개 Energy digest는 불변이다. saju-ts clean build가 runtime JSON pack을 dist에 누락하던 origin/main 선재 P1도 byte-identical copy와 dist import smoke로 닫았다. dist 없는 상태 build/import, release tools 5/5, saju-ts 58파일 481/481, npm pack의 `dist/schools/packs/builtin.pack.json` 포함을 확인했다. 이는 구조·패키징 계약이며 명리 수치 권위 승인이 아니다. |
...
| PR #653 리뷰 범위(역사 기록) | ✅ 25개 스택으로 대체·종료 | 6fb2f68a4 | 2026-07-12~14 | freeze `origin/main...6fb2f68a4`의 134커밋·418파일과 후속 backend 체크포인트를 커밋 재작성 없이 #654~#678에 배치했다. 현재 21개 Ready·4개 Draft이며 frontend source diff는 0이다. 누적 PR 자체는 닫혔다. |
