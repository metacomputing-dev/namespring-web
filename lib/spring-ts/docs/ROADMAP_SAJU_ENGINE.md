...
| Stack25 Seed 공개 질의·오류 계약 | ✅ 코드·통합 회귀 통과 / 🔶 스택 순서 대기 | d0b367581, PR #678 | 2026-07-14 | 공개 repository 질의의 타입·범위·enum·limit을 SQL 전에 검증하고, 오류는 안정 코드와 bounded `receivedSummary`만 노출한다. Energy singleton/score 입력과 1M whitespace 공격을 fail-closed로 막고 O(1) bounded code-point helper를 공유한다. Seed 전체 계약·DB/WASM·실제 package 5/5, Spring bridge·이름·후보·compat·snapshot을 통과했으며 유효 11,110개 Energy 시퀀스 digest `3225b6690600407ba4e0da6789678e325c5bec5262f7404a49f4e6ff006dcda5`는 불변이다. 이는 구조·보안 계약이며 명리 수치 권위 승인이 아니다. |
...
| PR #653 리뷰 범위(역사 기록) | ✅ 25개 Draft 스택으로 대체·종료 | 6fb2f68a4 | 2026-07-12~14 | freeze `origin/main...6fb2f68a4`의 134커밋·418파일과 후속 backend 체크포인트를 커밋 재작성 없이 #654~#678에 배치한다. Stack 25까지 main..Stack25는 159커밋이고 frontend source diff는 0이다. 누적 PR 자체는 닫혔다. |
