...
### 작업 5 — 인사이트 해석 고도화 (신살 위치 세분) ✅ 표준 세트 완비 (2026-07-06, PR #649)

- 구조는 완성 — **해석 파일에 entries만 추가하면 자동 반영** (`insight-registry` 조회 체인: 위치 세분 → 정확 factId → 타입 레벨 → 신살 백과).
- **인프라 (커밋 `e8c81e95d`)**: `withInterpretation`에 `preferredIds`(factId보다 먼저 시도) + 신살 루프가 **단일 히트일 때만** `shinsal.<이름>.<기둥>` 조회(다중 기둥 히트는 프론트가 factId 병합·위치 합산 → 위치 특정 문장이 오독이라 타입 레벨 유지). `hit.type`는 `SHINSAL_TYPE_KO_LABEL` 디스플레이값(도화·역마는 '살' 없음, 나머지 12신살은 '살' 포함)이라 factId 중간 세그먼트를 정확히 맞춰야 함.
- **위치 세분 완비 (커밋 `d457bd0a4`)**: `shinsal-positions.insights.json`에 **18신살 × 4기둥 = 72건**. 궁위 독법이 명리 **표준인 세트를 완비** — 12신살 전 12종(지살·도화·월살·망신살·장성살·반안살·역마·육해살·화개살·겁살·재살·천살) + 일간·록 계열(양인·홍염살·비인살·록신) + 대표 귀인(천을·문창). 록신은 궁별 고전 명칭(세록/건록/일록/귀록) 반영.
- **완료 판정 충족**: 데모 사주 위치 세분 채택 1→6건, 레지스트리 72/72 적재 일치, 재살·일주 프리뷰 end-to-end 렌더 확인.
- **의도적 비대상(스킵 = 명리 원칙)**: 귀인(천을·문창 외 ~16종)은 궁위 독법이 **부차적**(복의 종류가 본질 — 타입 레벨이 정답), 관계살(충·형·해·파·원진·격각)은 이미 `branchRelation`으로 노출되며 위치 세분이 의미 모호, 극(`stemRelation.극`)은 **타입 레벨 해석이 이미 존재해 렌더 갭 없음**. 즉 "일부만 위치별"이 아니라 위치 독법이 표준인 곳만 채운 것.
- **잔여(선택, 저우선)**: 귀인 궁위 세분을 굳이 원하면 파일 추가만으로 가능(구조 완비). 극 조합별(`stemRelation.극.<간지>`)도 폴리시 목적이면 추가 가능하나 갭은 아님.
