...
| 커밋 | 내용 | leaf 수 | 성격 |
|---|---|---|---|
| `3d65a56cf` | 스냅샷 도구 await 누락 수정 + 재캡처 (PR-1) | 63 | **계측기 수정** — main의 baseline은 고장난 도구(promise 미해결)로 캡처된 값. 전부 `candidatesTop5[].finalScore` 정정 |
...
이름 축 변화의 원천은 ① 계측기 수정(`3d65a56cf` — main 값 자체가 고장 캡처), ② 보완 오행
변화(조후·정자시설로 deficient/excessive가 바뀐 픽스처의 정당한 재산정), ③ 미세 가점
(`8401cfbab`·`a9f27f52b` — 순위 재배열 수준). 이름 점수의 절대값 이동은 κ 커버리지 계측
(로드맵 §2-4)에서 후퇴 0으로 확인됨.
