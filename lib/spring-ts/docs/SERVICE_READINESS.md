...
`validate:reference-authority` is the intake guard for top-level Reference A cases; it allows today's empty flat-case state, but blocks unresolved pages, low-tier authority truth, long summaries, and stored original prose when cases are added. `service:readiness` now includes this result as `authorityIntake`, and the strict paid gate requires zero intake violations.
