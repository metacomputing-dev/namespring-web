...
4. Re-run `npm run test:tiered-progressive-disclosure` when frontend copy or expansion behavior changes.
5. Re-run `npm run service:readiness` and `npm run service:readiness:paid-gate` before changing product copy around expert verification.
...
Use `authorityWorkBacklog` first. `P0_EXPERT_INTERNAL_EVIDENCE_REVIEW` rows are expert cells that already have deterministic internal numerical evidence, so they are the first places to attach reviewed Reference A or equivalent authority sources. Each backlog row includes `reviewExamples` with fragment ids and file paths so the source review can start from concrete copy.
...
`validate:reference-authority` is the intake guard for top-level Reference A cases; it allows today's empty flat-case state, but blocks unresolved pages, long summaries, stored original prose, and every authority claim rejected by the shared source-tier policy. Passing intake does not promote an observation case into an authority denominator. `service:readiness` includes this result as `authorityIntake`, and the strict paid gate requires zero intake violations.
