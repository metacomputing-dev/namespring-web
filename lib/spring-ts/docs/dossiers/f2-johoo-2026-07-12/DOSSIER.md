...
> 작성: 2026-07-12, Claude 감사 세션 (freeze `6fb2f68a4` 기준, 읽기 전용 조사).
> 재현: 이 폴더의 `verify-f2.mjs` (`node verify-f2.mjs` — 현재 checkout의 saju-ts `dist/` 빌드 필요). freeze에서 캡처한 5표본 기대값을 assertion으로 비교하며, 실행 중인 `dist/`가 어느 커밋에서 빌드됐는지까지 증명하는 provenance 도구는 아니다.
...
| 구분 | 내용 |
|---|---|
| ✅ **지금 해도 되는 것** | ① `yongshin.ts:313-316` 주석의 "조후위급 시 우선" → "조후위급 시 개입"으로 정정(사실 서술 교정 — 판정 무변경, 아래 §8 근거) ② baseline 픽스처 갭 보강 제안: **丑월 명식·신약×위급월(S4형) 픽스처 추가**(§6 — 판정 무변경, 감시 인프라) ③ 부록 A의 T2·T3·T4·T7(F2와 독립인 감사 티켓) |
...
**정리**: 조후위급이 교리적으로 가장 절실하다고 서술되는 극단 강약 영역(신약 s≲−0.28~−0.46, 신강 s≳0.59)에서 정확히 불가능하고, 반대로 **중화 근처에서는 조후가 이미 결정권을 가진다**(role 격차가 t≈0.545에서 0으로 압축 — fix-03 실증). 즉 현 기본값의 실제 동작은 "중화에서만 조후 결정, 극단에서 무력" — '위급 시 우선'의 **역방향**.
(상계 증명이므로 실제 불가능 영역은 표보다 넓다. 참고: 압박측 내부에서도 식상−재·관 pref차가 상수 0.2라 w_cli·Δcs_동측 ≈ 0.14 < 0.2×k — 신강 겨울명에서 조후 火(재)가 설기 木(식상)을 이기려면 def 차이 ≥ ~0.05가 추가로 필요, 표본 S5에서 실증.)

## 3–4. 고정 표본 5개 — 현재 점수와 최종 순위 (기본 config 실측)

| 표본 | 명식 | s(강약) | factor | 억부 1위 (bal+role) | 조후 오행 | 조후 순위 | 격차 λmin | 판정 |
|---|---|---|---|---|---|---|---|---|
| **fix-03**† | 己卯 丙子 戊午 壬子 (戊土 子月) | +0.020 (중화) | 0.404 | FIRE(0.349)=조후와 동일 | FIRE | **1위** | 0 | 중화라 조후 승리 가능 실증 |
| **fix-05**† | 乙酉 戊子 癸未 乙卯 (癸水 子月 '겨울 조후 케이스') | −0.085 | 0.404 | METAL 인성(Σ0.455) | FIRE | **2위** (Σ0.377) | 0.078 | 근소 패배 |
| **fix-06**† | 戊午 戊午 戊申 己未 (戊土 午月) | +0.806 (극신강) | 0.320 | METAL 식상(Σ0.788) | WATER | **2위** (Σ0.777) | 0.011 | 초근소 패배(재성이라 억부와 부분 호환) |
| **S4** | 丁巳 丙午 庚寅 丁丑 (庚金 午月 신약) | −0.416 | 0.320 | EARTH 인성(Σ0.619) | WATER | **3위** (Σ0.399) | 0.220 | §2 정리의 실증 — 궁통보감 취지(壬水 급선무)와 정면 충돌 |
...
- λ 실측 요구값(표본 역전 기준 λ ≥ λmin/factor): fix-05 **0.193** / fix-06 **0.034** / S4 **0.688** / S5 **0.097**. → λ≈0.25면 근소 케이스만 역전(S4 유지 — 신약 가드 효과), λ≈0.75면 전 표본 역전(A와 등가).
...
| 티켓 | 발견 | 위치 | 내용 | 상태 |
|---|---|---|---|---|
| T1 | F1 (P0) | facts.ts:2979·3028·3031·3042 | 격 선정에서 일간 자신이 '투간' 계상 → 甲日亥月·丙日寅月 GEONROK 오분류(실측). 일간 제외 + 회귀 픽스처. **판정 변경 — default-change 절차** | **코드 수정 완료 `0416c3daa`; exact default diff·명리 전문가 검토 대기** |
...
| T3 | F3 (P1) | facts.ts:2241·2262, gyeokgukSeongpae.ts:318-322, gyeokguk.ts:342-362 | 월지 형충 이중 감점(quality.multiplier × seongpae verdict 재곱) — monthBroken-단독 강등 시 배율 1.0 권장 | 대기 |
| T4 | F7 (P1) | yongshin.ts:502-516 | consensus confidence=0.35 상수·conflictLevel 항상 'high'(6/6 실측) — margin 정규화+임계 재보정 | 대기 |
| T5 | F6 (P1) | strengthFacts.ts:493-506·508-535·694-698 | 기반(羈絆) 감쇠 층위 불일치(비겁·인성=배율층 미미 vs 관살=base 차감) — 층위 통일+계측 | 대기 |
| T6 | F8 (P2) | springLegacy.ts:1494-1496·1940-1942 | finalHeesin=랭킹2위/gisin=최하위 기계 유도 — primaryMethod 인지형으로 (교리 검토 병행) | 대기 |
| T7 | F12 (P2) | facts.ts:3001-3026, gyeokguk.ts:574-616 등 | 무전거 magic 계수 인벤토리 문서화(코드 무변경) | 대기 |
