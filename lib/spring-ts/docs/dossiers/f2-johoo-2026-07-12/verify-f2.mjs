...
const ROOT = path.resolve(HERE, '../../../../saju-ts/src');
const u = (p) => pathToFileURL(path.join(ROOT, p)).href;
const { normalizeConfig } = await import(u('api/config.ts'));
const { pillar } = await import(u('core/cycle.ts'));
const { elementDistributionFromPillars } = await import(u('core/elementDistribution.ts'));
const { DEFAULT_SCORE_POLICY } = await import(u('core/scoring.ts'));
const { buildRuleFacts } = await import(u('rules/facts.ts'));
const { computeYongshin } = await import(u('rules/yongshin.ts'));
const { scorePillarsForRuleFacts } = await import(u('rules/ruleFactsScoring.ts'));
...
const EXPECTED = {
  'fix-03': { strength: 0.023, factor: 0.4039, weights: [0.8990, 0.8990, 0.3510], best: 'FIRE', ranking: 'FIRE>METAL>EARTH>WOOD>WATER', climateRank: 1, lambdaMin: 0.000 },
  'fix-05': { strength: -0.146, factor: 0.4039, weights: [0.8990, 0.8990, 0.3510], best: 'METAL', ranking: 'METAL>FIRE>WOOD>WATER>EARTH', climateRank: 2, lambdaMin: 0.139 },
  ...
  S4: { strength: -0.418, factor: 0.3200, weights: [0.9200, 0.9200, 0.3300], best: 'EARTH', ranking: 'EARTH>METAL>WATER>WOOD>FIRE', climateRank: 3, lambdaMin: 0.222 },
