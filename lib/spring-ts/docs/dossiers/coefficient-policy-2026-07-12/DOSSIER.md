...
| 구분 | 내용 |
|---|---|
| ✅ 착수 가능 | 미완 티켓 G1·G2·G3과 특성화 G5(§G). G4 경계 정책과 T7 계수 인벤토리는 완료 |
...
### G4. 경계값 가드 방향 정합
- **상태 (2026-07-18)**: **완료 (`4ffed280e`)**. 제안된 deDi/deShi 경계뿐 아니라 같은 결함군인 격국 root/shi norm, 관성 결합 penalty, 전왕 fallback도 `norm<=0 → 0`으로 통일했다. 0·음수·정상 기본값을 각각 회귀 고정했으며 기본 양수 경로는 기존 산출과 동일하다.
