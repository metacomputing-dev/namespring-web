...
| **S3** | **romance → family → academic** (사용자 지정 순서). ★실행 모드 전환: API 배치 대신 **세션 에이전트**(Fable 5 상속 → 한도 시 Opus → Codex 인계) — [S3_CONTINUATION_PLAYBOOK.md](./S3_CONTINUATION_PLAYBOOK.md) 참조 | 682 | 세션 사용량 (API $0) | 🔄 **romance 60번들/900편(33%) 완료** (2026-07-06 실측). 내역: 실제 S3 58번들/870편 = Codex 수동 45번들/675편(`regen-s3-codex-r1`~`r11`, r11 2번들 `regen-s3-codex-r11-fix1` 재보정) + `regen-manual-*` 13번들/195편 + S1 부수 2번들/30편(`regen-s1b/s1c-f5`). **잔여 1,800편.** family·academic은 **0% 미착수**(academic dir의 regen 30편은 S1 부수, academic 작업분 아님). 사용자 피드백 반영: 소량 재보정, 번역투·추상 결합 표현 제거, Codex 직접 작성·정독 후 반영 |
...
### 스타일 결정 (2026-07-05 사용자 확정)

- **일상 밀착형 구체 장면 유지** — "뭐 먹었는지 묻기" 같은 스몰토크·생활 팁 밀도는 설계 의도(다양성 계약+팔레트의 산물)이며 상품 차별점으로 확정. 격조 상향 다이얼은 쓰지 않는다.
