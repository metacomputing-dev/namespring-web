...
> `GUIDE_SAJU_ENGINE_IMPL.md` §0.5다. 누적 PR #653은 닫혔고 대체 스택에서 Ready와 merge gate를 분리한다. RPI는 20/100,
...
> 후보가 될 수 있다. 프로젝트 소유자는 2026-07-14 이 기준을 충족한 PR의 Ready 전환을 위임했으며, merge는 별도 승인한다.
...
- 운영 원칙: #654~#676을 재작성하지 않는다. Stack 23 코드 체크포인트는 `290b669a7`이고 frontend source diff는 0이다. exact-tip diff 완결·신규 코드 P0/P1 없음·회귀와 한계 공개를 충족하면 Ready로 전환할 수 있다. 선행 스택 순서, default/명리 승인, 실제 CI, authority/signoff는 merge gate로 유지한다.
